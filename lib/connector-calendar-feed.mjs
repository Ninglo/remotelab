...
export function filterCalendarSubscriptionChannelsForExposure(channels = {}) {
  ...
  const visibleVariant = variants.find((variant) => trimString(variant?.kind) === 'mainland')
    || variants.find((variant) => variant?.recommended)
    || variants[0]
    || null;
  ...
  return {
    ...
    mainlandHttpsUrl: trimString(visibleVariant?.kind) === 'mainland' ? visibleVariant.httpsUrl : '',
    mainlandWebcalUrl: trimString(visibleVariant?.kind) === 'mainland' ? visibleVariant.webcalUrl : '',
    publicHttpsUrl: '',
    publicWebcalUrl: '',
