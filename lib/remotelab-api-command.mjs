...
export async function runRemoteLabApiCommand(argv = [], io = {}) {
  ...
  const sourceSessionId = trimString(process.env.REMOTELAB_SESSION_ID);
  const result = await client.request(options.path, {
    method: options.method,
    body,
    ...(sourceSessionId ? {
      headers: { 'X-RemoteLab-Source-Session-Id': sourceSessionId },
    } : {}),
  });
