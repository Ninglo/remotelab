...
export function normalizeScheduledSessionTemplate(value, fallbackTool = '', legacy = {}) {
  ...
  if (raw.reuse === 'fixed_session') {
    const sessionId = trim(raw.sessionId);
    if (!sessionId) throw new Error('Fixed Session execution requires sessionTemplate.sessionId');
    template.reuse = 'fixed_session';
    template.sessionId = sessionId;
  } else if (raw.reuse === 'calendar_day') {
...
export function scheduledSessionIdentity(trigger, template) {
  if (template.reuse === 'fixed_session') return `fixed-session:${template.sessionId}`;
