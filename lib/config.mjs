...
export const MAINLAND_PUBLIC_BASE_URL = normalizeBaseUrl(process.env.REMOTELAB_MAINLAND_PUBLIC_BASE_URL)
  || deriveGuestMainlandPublicBaseUrl(process.env.REMOTELAB_GUEST_MAINLAND_BASE_URL, INSTANCE_ROOT);

function deriveGuestMainlandPublicBaseUrl(mainlandRootBaseUrl, instanceRoot) {
  const normalizedRoot = normalizeBaseUrl(mainlandRootBaseUrl);
  if (!normalizedRoot) return '';
  const instanceName = trimString(basename(instanceRoot || ''));
  if (!instanceName) return normalizedRoot;
  return appendUrlPath(normalizedRoot, instanceName);
}
...
export const SESSION_SPAWN_PROMPT_ENABLED = envFlagEnabled(
  process.env.REMOTELAB_SESSION_SPAWN_PROMPT,
  !INSTANCE_ROOT,  // off by default for non-default instances (trial, demo, etc.)
);
