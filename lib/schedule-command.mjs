...
function parseArgs(argv = []) {
  ...
  const options = {
    ...
    sessionId: command === 'list' ? '' : trimString(process.env.REMOTELAB_SESSION_ID),
  ...
  for (let index = consumesId ? 2 : 1; index < argv.length; index += 1) {
    ...
    else if (arg === '--runtime-policy') options.runtimePolicy = take();
