...
assert.match(CREATE_AGENT_STARTER_SYSTEM_PROMPT, /Low-loss, reversible, auditable actions must execute by default/);
assert.match(CREATE_AGENT_STARTER_SYSTEM_PROMPT, /run a clean-room dry-run in a newly created Agent session/);
assert.match(CREATE_AGENT_STARTER_SYSTEM_PROMPT, /exercise sandboxed, reversible, or explicitly self-targeted side effects/);
assert.doesNotMatch(CREATE_AGENT_STARTER_SYSTEM_PROMPT, /Review Gates are binding interaction checkpoints/);
