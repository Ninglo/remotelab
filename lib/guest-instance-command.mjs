...
function printHelp(stdout = process.stdout) {
  stdout.write([
    ...
    '  --isolated               Create with dedicated OS user + systemd sandbox (Linux only)',
...
function parseArgs(argv = []) {
  ...
  let startIndex = new Set(['create', 'show', 'status', 'links', 'check', 'report', 'converge', 'audit-migration']).has(options.command)
    ? (trimString(options.name) ? 2 : 1)
    : 1;
  if (new Set(['create', 'show', 'status', 'links', 'check', 'report', 'converge', 'audit-migration']).has(options.command) && options.name.startsWith('-')) {
    options.name = '';
    startIndex = 1;
  }
...
async function writeLinuxGuestEnvironmentFile(name, environmentVariables = {}, { restrictPermissions = false } = {}) {
...
async function seedIsolatedToolCredentials(homePath, userName) {
  // Clone Claude Code credentials if available
  const ownerClaudeDir = join(HOME_DIR, '.claude');
  const guestClaudeDir = join(homePath, '.claude');
  ...
  const guestCodexDir = join(homePath, '.codex');
  ...
  if (await pathExists(ownerCodexAuth)) {
    ...
    await copyFile(ownerCodexAuth, join(guestCodexDir, 'auth.json'));
    const ownerCodexConfig = join(ownerCodexDir, 'config.toml');
    if (await pathExists(ownerCodexConfig)) {
      await copyFile(ownerCodexConfig, join(guestCodexDir, 'config.toml'));
    }
  }

  // Set ownership of everything under the guest home to the isolated user
  await execOrThrow('chown', ['-R', `${userName}:${userName}`, homePath],
    `Setting ownership for ${homePath}`);
}
...
async function provisionIsolatedInstance(instanceName) {
  ...
  await seedIsolatedToolCredentials(homePath, userName);
...
function buildGuestWeChatConnectorEnv({ instanceRoot = '', port = 0, publicBaseUrl = '' } = {}) {
  ...
  env.HOME = trimString(env.HOME) || HOME_DIR;
  env.PATH = trimString(env.PATH) || '/usr/local/bin:/usr/bin:/bin';
...
async function discoverIsolatedLaunchAgents() {
  ...
  if (process.platform !== 'darwin') {
    ...
    for (const record of registry) {
      ...
      const instanceRoot = trimString(environmentVariables.REMOTELAB_INSTANCE_ROOT)
        || trimString(record?.instanceRoot)
...
async function createSingleGuestInstance(options) {
  if (process.platform !== 'darwin') {
    ...
    const envFile = await writeLinuxGuestEnvironmentFile(name, {
      ...
      HOME: isolated ? instanceHome : HOME_DIR,
    ...
    try {
      ...
      await startGuestWeChatConnector({ instanceRoot, port, publicBaseUrl });
    } catch (error) {
      warnings.push(`WeChat connector autostart failed for ${name}: ${error instanceof Error ? error.message : String(error)}`);
    }
...
async function convergeGuestInstances(options) {
  ...
  for (const target of matchingTargets) {
    ...
    // Re-sync isolation override and tool credentials for isolated instances
    if (isolated && !options.dryRun) {
      await writeIsolationOverride(target.name);
      const isolatedHome = buildIsolatedHomePath(target.name);
      const isolatedUser = buildIsolatedUserName(target.name);
      await seedIsolatedToolCredentials(isolatedHome, isolatedUser);
      result.isolationSynced = true;
    }

    if ((drift.changed || buildDriftChanged) && !options.dryRun && !options.noRestart) {
      await restartGuestService(target, { reloadDefinition: drift.changed });
