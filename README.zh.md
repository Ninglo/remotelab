...
现在这条大陆桥统一走 prefix-only：所有产品入口都放在 `https://<mainland-base>/<name>/` 下，包括 owner 主服务（默认 `https://<mainland-base>/owner/`，本机端口 `7690`）。裸根路径只保留成一个中性的路由索引页，不再偷偷映射到某个实例，避免把已经在用的入口静默切走。想改主服务这条命名前缀时，用 `NATAPP_MAINLAND_SERVICE_NAME` / `NATAPP_MAINLAND_SERVICE_PORT`；旧的 `NATAPP_OWNER_ROUTE_PREFIX` / `NATAPP_OWNER_UPSTREAM_PORT` 仍然兼容。路由原则和这次清理背景见 [docs/mainland-routing.md](docs/mainland-routing.md)。
