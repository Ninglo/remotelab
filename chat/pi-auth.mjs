...
export async function installPiCredentialFromDeviceLogin({
  loginAuthFile,
  piAuthFile = join(PI_AGENT_DIR, 'auth.json'),
} = {}) {
  if (!trimString(loginAuthFile)) throw new Error('Pi device login result is required');
  const loginAuth = await readJsonObject(loginAuthFile);
  const access = trimString(loginAuth?.tokens?.access_token);
  const refresh = trimString(loginAuth?.tokens?.refresh_token);
  const accountId = trimString(loginAuth?.tokens?.account_id);
  ...
  async function startDeviceLogin({ restart = false } = {}) {
    ...
    activeChild.on('close', (code) => {
      ...
      void credentialQueue(async () => {
        await installPiCredentialFromDeviceLogin({
          loginAuthFile: join(runtime.loginHome, 'auth.json'),
