...
async function main() {
  ...
  try {
    ...
    assert.match(combinedChatStyles, /\.chat-area\s*\{[\s\S]*?grid-template-rows:\s*minmax\(0, 1fr\) auto auto;[\s\S]*?min-height:\s*0;/, 'chat area should model transcript, queued panel, and composer as explicit rows');
