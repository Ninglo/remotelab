...
export function createFeishuConnectorTransport({
  ...
  const sendPartOnce = async (key, sender) => {
    ...
    async send(message) {
      try {
        ...
        const text = renderOutboundChatMessageText(message, {
          includeAttachmentFallback: includeAttachmentFallback && typeof sendFeishuAttachmentImpl !== 'function',
        });
        const responses = [];
        if (text) {
          responses.push(await sendPartOnce(`${baseKey}:text`, () => sendFeishuTextImpl(
            runtime,
            summary,
            text,
            `${baseKey}:text`,
        ...
        if (typeof sendFeishuAttachmentImpl === 'function') {
          for (let index = 0; index < attachments.length; index += 1) {
            responses.push(await sendPartOnce(`${baseKey}:attachment:${index}`, () => sendFeishuAttachmentImpl(
              runtime,
              summary,
              attachments[index],
