...
export async function loadUpgradePlan(path) {
  ...
  if (
    input.preflightMode !== undefined &&
    !["online", "offline"].includes(input.preflightMode)
  )
    throw new Error("preflightMode must be online or offline");
  if (
...
export async function checkHttp(plan, which) {
  const token = await readServiceToken(join(plan.configDir, "auth.json")).catch(() => "");
  if (!token)
    throw new Error("Cannot verify instance health without its service token");
