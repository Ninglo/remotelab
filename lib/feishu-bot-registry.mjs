...
function parseConfigArgumentList(args = []) {
  for (let index = 0; index < args.length; index += 1) {
    const arg = trimString(args[index]);
    if (arg === '--config') return trimString(args[index + 1]);
    if (arg.startsWith('--config=')) return trimString(arg.slice('--config='.length));
  }
  return '';
}
...
export function parseFeishuLaunchdPlist(plist = {}, runtimeOutput = '', options = {}) {
  ...
  const programArguments = Array.isArray(plist?.ProgramArguments)
    ? plist.ProgramArguments.map((value) => String(value))
    : trimString(plist?.Program)
      ? [trimString(plist.Program)]
      : [];
  const label = trimString(plist?.Label);
  const uid = Number.isInteger(options.uid) ? options.uid : process.getuid?.();
  const stateMatch = String(runtimeOutput || '').match(/^\s*state\s*=\s*(\S+)\s*$/m);
  const pidMatch = String(runtimeOutput || '').match(/^\s*pid\s*=\s*(\d+)\s*$/m);
  const defaultConfigPath = normalizeConfigPath(
    options.defaultConfigPath
      || systemdDefaultConfigPath(environment, DEFAULT_CONFIG_PATH),
  );
  return {
    kind: 'launchd',
    label,
    target: label && Number.isInteger(uid) ? `gui/${uid}/${label}` : '',
    plistPath: normalizeConfigPath(options.plistPath),
    activeState: trimString(stateMatch?.[1]) || 'stopped',
    pid: parsePid(pidMatch?.[1]),
    command: programArguments.join(' '),
    configPath: normalizeConfigPath(
      parseConfigArgumentList(programArguments)
        || systemdDefaultConfigPath(environment, defaultConfigPath),
    ),
  };
}
...
async function collectLaunchdFacts(defaultConfigPath) {
  ...
  for (const entry of entries) {
    ...
    const plistPath = join(launchAgentsDir, entry.name);
    try {
      ...
      const programArguments = Array.isArray(plist?.ProgramArguments)
        ...
        : [plist?.Program];
      if (!programArguments.some((value) => /scripts\/feishu-connector\.mjs$/.test(trimString(value)))) {
        continue;
      }
      const label = trimString(plist?.Label);
      if (!label) continue;
      ...
      let runtimeOutput = '';
      try {
        const result = await execFile('launchctl', ['print', target], {
          maxBuffer: 1024 * 1024,
        });
        runtimeOutput = result.stdout;
      } catch {
        // A configured LaunchAgent can be unloaded or stopped.
      }
      facts.push(parseFeishuLaunchdPlist(plist, runtimeOutput, {
        defaultConfigPath,
        plistPath,
        uid,
      }));
    } catch {
      // Ignore unrelated or malformed property lists.
    }
  }
  return facts;
}
...
function buildRuntime(configPath, processFacts, systemdFacts, launchdFacts) {
  const matchingSystemd = systemdFacts.filter((fact) => fact.configPath === configPath);
  const matchingLaunchd = launchdFacts.filter((fact) => fact.configPath === configPath);
  const servicePids = new Set(
    [...matchingSystemd, ...matchingLaunchd].map((fact) => fact.pid).filter(Boolean),
  );
  const matchingProcesses = processFacts.filter(
    (fact) => fact.configPath === configPath && !servicePids.has(fact.pid),
  );
  const owners = [...matchingSystemd, ...matchingLaunchd, ...matchingProcesses];
  ...
  if (matchingLaunchd.length === 1) {
    const fact = matchingLaunchd[0];
    const running = fact.activeState === 'running' && fact.pid > 0;
    return {
      status: running ? 'running' : 'stopped',
      runtime: {
        kind: 'launchd',
...
export async function discoverFeishuBots(options = {}) {
  ...
  const launchdFacts = Array.isArray(options.launchdFacts)
    ? options.launchdFacts
    : await collectLaunchdFacts(defaultConfigPath);
  ...
  const configPaths = new Set([
    ...
    ...launchdFacts.map((fact) => normalizeConfigPath(fact.configPath)),
  ].filter(Boolean));
  ...
  for (const configPath of [...configPaths].sort()) {
    ...
    const runtimeState = buildRuntime(configPath, processFacts, systemdFacts, launchdFacts);
...
export function buildFeishuBotRestartPlan(bot, options = {}) {
  ...
  if (
    ...
    && trimString(bot.runtime.target)
    && trimString(bot.runtime.plistPath)
  ) {
    return {
      kind: 'launchd',
      command: 'launchctl',
      args: ['kickstart', '-k', bot.runtime.target],
      plistPath: bot.runtime.plistPath,
    };
  }
