import { CONFIG_DIR, INSTANCE_NAME, IS_GUEST_INSTANCE } from './config.mjs';
import {
...
import { runOwnerInstanceConnectorCommand } from './owner-instance-connector-command.mjs';
...
function printHelp(stdout) {
  ...
  remotelab connector list --instance <managed-guest> [--json]
  remotelab connector call <channel:skill> [--parameter value ...] [--json]
  remotelab connector call <channel:skill> --instance <managed-guest> [--parameter value ...] [--json]

Example:
  remotelab connector call wechat:send_text --text "Reminder text" --json
  remotelab connector call wechat:send_text --instance trial8 --text "Remember to drink water" --json
...
command. The owner runtime may target a managed local guest with --instance;
guest runtimes cannot use that override. Cross-instance owner calls execute in
the target instance and are recorded in the owner admin audit log. General
provider APIs such as Feishu Docs, Wiki, Base, Sheets, and Drive belong to the
instance runtime and its native CLI tools, not Connector actions. Calls made
inside a connector-backed session automatically use that session's source route.
`);
}

function removeOption(argv = [], optionName = '') {
  const flag = `--${trimString(optionName)}`;
  const filtered = [];
  for (let index = 0; index < argv.length; index += 1) {
    if (argv[index] !== flag) {
...
export async function runConnectorCommand(argv = [], io = {}) {
  ...
  const targetInstance = trimString(options.instance);
  if (options.instance === true) throw new Error('--instance requires a managed guest instance name');
  if (targetInstance) {
    if (IS_GUEST_INSTANCE) {
      throw new Error('--instance is owner-only; guest instances cannot invoke another instance Connector');
    }
    if (!new Set(['list', 'call']).has(command)) {
      throw new Error('--instance is only supported by connector list and connector call');
    }
    const runTarget = io.runOwnerInstanceConnectorCommand || runOwnerInstanceConnectorCommand;
    const delegated = await runTarget(targetInstance, removeOption(argv, 'instance'));
    if (delegated.stdout) stdout.write(delegated.stdout);
    if (delegated.stderr) stderr.write(delegated.stderr);
    return Number.isInteger(delegated.exitCode) ? delegated.exitCode : 0;
  }
  ...
  const reserved = new Set(['json', 'help', 'session-id', 'instance', 'instance-id', 'source-route-id']);
  const parameters = {};
