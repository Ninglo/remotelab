...
- Background model calls such as session title/group summarizers must resolve the same machine-account Codex home and provider-auth selection as foreground runs.
...
- Keep Codex-home selection in one shared resolver and regression-test both foreground and background paths with an explicit machine-level override.
...
- Fingerprinted assets are still not enough if the shipped server mixes live-edited frontend files into a long-lived process without a clear restart boundary; backend changes should take effect on restart, while frontend assets still need coherent versioning.
- For small single-owner self-hosted deployments, a simpler stable default is one source-backed runtime plus clean restart recovery. Add a separate immutable release/snapshot layer only when staged rollout or rollback across multiple externally visible surfaces is genuinely worth the complexity.
...
- Treat an instance—not a Session—as the user isolation boundary. One user's instance may contain many Sessions for separate work contexts; unrelated users need separate instance config, memory, workspace, auth, Connector bindings, and runtime permissions.
