...
assert.match(templateSource, /Auto follows the current browser language\./);
...
assert.match(i18nSource, /"settings\.language\.note": "Auto follows the current browser language\./);
assert.match(i18nSource, /"settings\.sessionPresentation\.note": "Internal-only display control for the selected session\./);
...
assert.match(i18nSource, /"settings\.language\.note": "默认会跟随当前浏览器语言。你也可以在这里为当前浏览器强制切换/);
