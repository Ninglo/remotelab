...
const tempHome = mkdtempSync(join(tmpdir(), 'remotelab-title-refresh-manual-lock-'));
...
if (isWorkstreamPrompt) {
  ...
  } else if (prompt.includes('Redesign the sidebar grouping logic')) {
    text = JSON.stringify(buildPayload(
      'Sidebar Grouping',
      'Sidebar Grouping',
...
try {
  ...
  await waitFor(
    ...
    'session should receive the initial automatic title',
  );

  const manuallyRenamed = await renameSession(session.id, 'Locked Manual Title');
  assert.equal(manuallyRenamed?.name, 'Locked Manual Title');
  assert.equal(manuallyRenamed?.titleLocked, true, 'manual rename should lock the title');
  ...
  await waitFor(
    async () => {
      const current = await getSession(session.id);
      return current?.name === 'Locked Manual Title'
        && current?.group === 'Sidebar Grouping'
        && current?.description === 'Redesign the session grouping rules for the sidebar.'
        && current?.titleLocked === true
        && current?.activity?.run?.state === 'idle';
    },
    'manual title lock should block later automatic title refresh while still allowing grouping refresh',
  );
