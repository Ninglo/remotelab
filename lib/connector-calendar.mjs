...
export async function createCalendarEvent({ bindingId, credentialsPath, title, startTime, endTime, description, location, calendarId, timezone } = {}) {
  const binding = await resolveCalendarConnectorBinding({ bindingId });
  const bindingError = resolveBindingError(binding);
...
export async function updateCalendarEvent({ bindingId, credentialsPath, eventId, title, startTime, endTime, description, location, calendarId, timezone } = {}) {
  const binding = await resolveCalendarConnectorBinding({ bindingId });
  const bindingError = resolveBindingError(binding);
  if (bindingError) return bindingError;
