...
如果你确实还保留着一条按路径进入的 bridge，可以把共享根地址写进 `~/.config/remotelab/guest-instance-defaults.json`：
...
配置好之后，`links` 和报告里仍然可以带出这条 bridge 入口，但新实例默认不会再把这类 path-prefix 入口写进自己的运行时环境。只有还需要这条 bridge 的部署才应该继续这样用。

如果某个部署还保留这条 bridge，再用 `NATAPP_BRIDGE_SERVICE_NAME` / `NATAPP_BRIDGE_SERVICE_PORT` 去控制它。路由原则见 [docs/prefix-bridge-routing.md](docs/prefix-bridge-routing.md)。
