...
import { AUTH_FILE, CONFIG_DIR } from './config.mjs';
...
export const WECHAT_LOGIN_OPEN_PATH = '/connectors/wechat/login/open';
...
function buildQrVersion(loginState) {
  const normalized = trimString(loginState?.qrcodeUrl || loginState?.loginId);
  if (!normalized) return '';
  return createHash('sha1').update(normalized).digest('hex').slice(0, 12);
}

function buildLoginOpenGrantId(loginState) {
  return trimString(loginState?.loginId || loginState?.qrcodeUrl);
}

function normalizeLoginOpenSigningPath(openPath) {
  const normalizedPath = trimString(openPath) || WECHAT_LOGIN_OPEN_PATH;
  const markerIndex = normalizedPath.lastIndexOf(WECHAT_LOGIN_OPEN_PATH);
  if (markerIndex >= 0) {
...
async function loadLoginOpenSecret() {
  const authDocument = await readJson(AUTH_FILE, {});
  return trimString(authDocument?.token);
}

async function signLoginOpenPath(openPath, loginState) {
  const normalizedOpenPath = normalizeLoginOpenSigningPath(openPath);
  const grantId = buildLoginOpenGrantId(loginState);
  const secret = await loadLoginOpenSecret();
  if (!grantId || !secret) return null;
  return createHmac('sha256', secret)
    .update(`${normalizedOpenPath}\n${grantId}`)
    .digest('hex')
    .slice(0, 32);
}

export async function buildWeChatLoginOpenPath({
  openPath = WECHAT_LOGIN_OPEN_PATH,
  loginState = null,
} = {}) {
  const normalizedOpenPath = trimString(openPath) || WECHAT_LOGIN_OPEN_PATH;
  const grantId = buildLoginOpenGrantId(loginState);
  const signature = await signLoginOpenPath(normalizedOpenPath, loginState);
  if (!grantId || !signature) return normalizedOpenPath;
  const url = new URL(normalizedOpenPath, 'http://127.0.0.1');
  url.searchParams.set('lid', grantId);
  url.searchParams.set('sig', signature);
  return `${url.pathname}${url.search}`;
}

export async function verifyWeChatLoginOpenRequest({
  pathname = WECHAT_LOGIN_OPEN_PATH,
  searchParams = null,
} = {}) {
  const normalizedPathname = normalizeLoginOpenSigningPath(pathname);
  const params = searchParams instanceof URLSearchParams ? searchParams : new URLSearchParams(searchParams || '');
  const providedGrantId = trimString(params.get('lid'));
  const providedSignature = trimString(params.get('sig')).toLowerCase();
  if (!providedGrantId || !providedSignature) return false;

  const loginState = await loadWeChatLoginState();
  const expectedGrantId = buildLoginOpenGrantId(loginState);
  if (!expectedGrantId || providedGrantId !== expectedGrantId) return false;

  const expectedSignature = await signLoginOpenPath(normalizedPathname, loginState);
...
export async function getWeChatLoginSurface({
  ...
  const qrcodeVersion = buildQrVersion(loginState);
  const status = linkedAccount
    ? 'connected'
    : (trimString(loginState?.status).toLowerCase() || (loginPid ? 'starting' : 'starting'));
  const loginLinkPath = await buildWeChatLoginOpenPath({
    openPath,
    loginState,
  });
