...
Examples:
  remotelab connector call feishu:document_get --document-token <token> --json

Connector calls are instance-scoped. Provider credentials remain inside the
registered connector process and are never printed by this command.
`);
}
...
export async function runConnectorCommand(argv = [], io = {}) {
  ...
  const reserved = new Set(['json', 'help', 'session-id', 'instance-id']);
  const parameters = {};
  ...
  const result = await executeConnectorSkill(toolName, parameters, {
    ...
    sessionId: trimString(options['session-id']) || trimString(process.env.REMOTELAB_SESSION_ID),
  });
