...
export async function updateCalendarFeedEvent(uid, updates) {
  return await feedMutationQueue(CALENDAR_EVENTS_FILE, async () => {
    ...
    if (updates.reminderMinutesBefore !== undefined) {
      event.reminderMinutesBefore = normalizeReminderMinutes(updates.reminderMinutesBefore);
    }
...
export function generateIcsContent(doc) {
  ...
  for (const event of doc.events || []) {
    ...
    if (Number.isInteger(event.reminderMinutesBefore) && event.reminderMinutesBefore >= 0) {
