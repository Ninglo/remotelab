...
import { BRIDGE_PUBLIC_BASE_URL, CALENDAR_EVENTS_FILE, PUBLIC_BASE_URL } from './config.mjs';
...
export const CALENDAR_SUBSCRIBE_HELPER_PATH = '/subscribe/calendar';

function trimString(value) {
  return typeof value === 'string' ? value.trim() : '';
}

function normalizeSubscriptionBaseUrl(baseUrl, { allowLocalhost = false } = {}) {
...
export function buildSubscriptionUrl(baseUrl, feedToken, options = {}) {
  const base = normalizeSubscriptionBaseUrl(baseUrl, options);
  if (!base || !trimString(feedToken)) return '';
  return `${base}/cal/${feedToken}.ics`;
}

export function buildWebcalSubscriptionUrl(baseUrl, feedToken, options = {}) {
  const httpsUrl = buildSubscriptionUrl(baseUrl, feedToken, options);
  if (!httpsUrl) return '';
  return httpsUrl.replace(/^https?:\/\//, 'webcal://');
}

export function buildCalendarSubscribeHelperPath({ format = 'webcal' } = {}) {
  return trimString(format).toLowerCase() === 'https'
    ? `${CALENDAR_SUBSCRIBE_HELPER_PATH}?format=https`
...
export function buildCalendarSubscriptionChannels({
  ...
  const alternateList = Array.isArray(alternateBaseUrls) ? alternateBaseUrls : [alternateBaseUrls];
  const candidates = [
    {
      kind: 'primary',
      label: 'Current',
  ...
  for (const candidate of candidates) {
    addSubscriptionVariant(variants, seenUrls, {
      ...
      recommended: candidate.recommended && variants.length === 0,
    });
  }
...
export function filterCalendarSubscriptionChannelsForExposure(channels = {}) {
  const variants = Array.isArray(channels?.variants) ? channels.variants : [];
  const visibleVariant = variants.find((variant) => variant?.recommended)
    || variants[0]
    || null;

  return {
    preferredHttpsUrl: visibleVariant?.httpsUrl || '',
    preferredWebcalUrl: visibleVariant?.webcalUrl || '',
    variants: visibleVariant ? [visibleVariant] : [],
  };
}
...
export function getDefaultCalendarSubscriptionChannels(feedToken = '') {
  return buildCalendarSubscriptionChannels({
    feedToken,
    primaryBaseUrl: PUBLIC_BASE_URL,
    alternateBaseUrls: BRIDGE_PUBLIC_BASE_URL ? [BRIDGE_PUBLIC_BASE_URL] : [],
  });
}
...
export async function dispatchCalendarToFeed(target, { sessionId, runId, baseUrl } = {}) {
  ...
  const channels = filterCalendarSubscriptionChannelsForExposure(buildCalendarSubscriptionChannels({
    feedToken,
    primaryBaseUrl: trimString(baseUrl),
