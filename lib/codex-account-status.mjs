...
export async function readCodexAccountStatus({
  command, env, includeRateLimits = false, spawnProcess = spawn,
  timeoutMs = includeRateLimits ? 45000 : 12000,
  signal,
}) {
  const before = await readCodexAuthMetadata(env.CODEX_HOME);
  const result = await new Promise((resolve, reject) => {
    let child, timer, killTimer, forceSettleTimer, buffer = '', settled = false, stopping = false, account = null;
    let outcome = null;
    const settle = () => {
      ...
      else resolve({ account, usage: outcome.usage });
    };
    const finish = (error, usage = null) => {
      ...
      killTimer.unref?.();
      forceSettleTimer = setTimeout(settle, 2000);
    ...
    const send = message => {
      if (!settled && !stopping) child.stdin.write(JSON.stringify(message) + '\n');
    };
    if (signal?.aborted) {
      abort();
      return;
    ...
    try {
      ...
      child.on('close', () => {
        if (stopping) settle();
        else finish('Codex account connection closed');
      });
