...
const DEFAULT_SESSION_DISPATCH_SETTING = 'off';
...
const PLATFORM_SKILL_DEFINITIONS = Object.freeze([
  ...
  },
  {
    ...
    title: 'Stable Static Publish',
    triggers: ['网页预览', '静态网页', 'demo 链接', 'Pages 部署', 'static site'],
    description: 'Publish static build output to a fixed hosting project; reserve Guest Port Expose for dynamic services.',
  ...
  },
  {
    ...
    description: 'Publish a guest-owned loopback port through a controlled `<instance>-<label>.<domain>` hostname.',
...
function buildConvergedLaunchAgentSpec(agent = {}, options = {}) {
  ...
  environmentVariables.REMOTELAB_CODEX_HOME_MODE = 'personal';
  environmentVariables.REMOTELAB_SESSION_DISPATCH = DEFAULT_SESSION_DISPATCH_SETTING;
...
function detectLaunchAgentConvergence(agent = {}, next = buildConvergedLaunchAgentSpec(agent)) {
  ...
  const sessionDispatchChanged = trimString(currentEnv.REMOTELAB_SESSION_DISPATCH)
    !== trimString(next.environmentVariables?.REMOTELAB_SESSION_DISPATCH);
  const codexHomeModeChanged = trimString(currentEnv.REMOTELAB_CODEX_HOME_MODE)
    !== trimString(next.environmentVariables?.REMOTELAB_CODEX_HOME_MODE);
  const machineCodexHomeChanged = trimString(currentEnv.REMOTELAB_MACHINE_CODEX_HOME)
    !== trimString(next.environmentVariables?.REMOTELAB_MACHINE_CODEX_HOME);
  ...
  return {
    changed: codePathChanged
      ...
      || sessionDispatchChanged
      || codexHomeModeChanged
      || machineCodexHomeChanged
      || instanceRuntimeEnvironmentChanged
      || sharedCodexEnvPresent
...
async function createSingleGuestInstance(options) {
  if (process.platform !== 'darwin') {
    ...
    const envFile = await writeLinuxGuestEnvironmentFile(name, {
      ...
      PATH: guestPathValue,
      REMOTELAB_CODEX_HOME_MODE: 'personal',
      REMOTELAB_SESSION_DISPATCH: DEFAULT_SESSION_DISPATCH_SETTING,
