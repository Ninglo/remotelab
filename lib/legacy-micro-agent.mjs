...
import { isRuntimeModelAllowed } from './model-availability-policy.mjs';
...
export function normalizeCodexModelId(value, defaultModel = PRODUCT_DEFAULT_CODEX_MODEL) {
  ...
  const candidate = isStaleCodexModelId(normalized, defaultModel)
    ? trimString(defaultModel)
    : normalized;
  return isRuntimeModelAllowed(PRODUCT_DEFAULT_TOOL_ID, candidate)
    ? candidate
    : trimString(defaultModel);
}

export function normalizeRuntimeModelForTool(tool, model) {
  const normalizedTool = normalizeLegacyToolId(tool);
  const normalizedModel = trimString(model);
  if (normalizedTool === PRODUCT_DEFAULT_TOOL_ID) {
    return normalizeCodexModelId(normalizedModel);
  }
  if (normalizedTool !== 'pi') return normalizedModel;

  const prefix = 'openai-codex/';
  if (normalizedModel.startsWith(prefix)) {
    return `${prefix}${normalizeCodexModelId(normalizedModel.slice(prefix.length))}`;
  }
  return normalizeCodexModelId(normalizedModel);
}
...
export function migrateLegacySessionRuntimeFields(value = {}) {
  if (!isLegacyMicroAgentToolId(value?.tool)) {
    const tool = normalizeLegacyToolId(value?.tool);
    return {
      ...value,
      tool,
      model: normalizeRuntimeModelForTool(tool, value?.model),
...
export function migrateLegacyRuntimeSelection(value = {}) {
  if (!isLegacyMicroAgentToolId(value?.selectedTool)) {
    ...
    return {
      ...value,
      selectedTool,
      selectedModel: normalizeRuntimeModelForTool(selectedTool, value?.selectedModel),
      selectedEffort: clampReasoningEffort(value?.selectedEffort),
    };
  }
  return {
    ...value,
