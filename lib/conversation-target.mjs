...
export function sameConversation(left, right) {
  ...
  if (a.connector === 'feishu') {
    ...
    const xTopicIds = topicIds(x), yTopicIds = topicIds(y);
    // Ordinary unthreaded group messages share the long-lived continue-mode
    // conversation for that Bot and chat. A target without a messageId stays
    // an independent publication destination, while topics still match only
    // through their explicit topic/root identities below.
    if (!xTopicIds.length && !yTopicIds.length && x.messageId && y.messageId) return true;
    const ids = new Set(xTopicIds);
    return yTopicIds.some(id => ids.has(id));
  }
...
// A Session may keep one logical conversation while an individual inbound
// request replies to a newer message inside that same transport scope. This is
// deliberately broader than sameConversation for Feishu groups: different
// topics in one chat are different conversations, but they are still within
// the same safe per-request delivery scope.
export function sameConversationScope(left, right) {
  const a = normalizeConversation(left), b = normalizeConversation(right);
  if (!a || !b || a.connector !== b.connector || a.sourceRouteId !== b.sourceRouteId) return false;
  const x = a.target, y = b.target;
  if (a.connector === 'feishu') {
    if (x.tenantKey && y.tenantKey && x.tenantKey !== y.tenantKey) return false;
    if (x.commentId || y.commentId) {
      return Boolean(x.fileToken) && x.fileToken === y.fileToken && x.commentId === y.commentId;
    }
    return Boolean(x.chatId) && x.chatId === y.chatId;
  }
  if (a.connector === 'wechat') return x.accountId === y.accountId && x.peerUserId === y.peerUserId;
  return x.to === y.to && Boolean(x.threadId) && x.threadId === y.threadId;
}
...
export function conversationAfterReceipt(value, receipt = {}) {
  ...
  // An inbound message id without a topic marker is an ordinary group-chat
  // reply address. Its outbound receipt must not silently turn continue mode
  // into a new thread. Only an unaddressed group publication learns a topic
  // from its receipt; explicit thread replies retain their existing root.
  if (target.messageId && !topicIds(target).length) return conversation;
