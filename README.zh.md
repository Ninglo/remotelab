...
你可以用手机浏览器去控制 Claude Code、Codex、Cline 以及兼容的本地工具。RemoteLab 不是终端模拟器，也不是移动 IDE；它更像一个持久化的 chat/control plane，会把 session、run 和历史状态落到磁盘上。
...
最快的方式仍然是：把一段 setup prompt 粘贴给部署机器上的 Claude Code。它可以自动完成绝大多数步骤，只会在 Cloudflare 登录这类真正需要人工参与的地方停下来。

**粘贴前的前置条件：**
- **macOS**：已安装 Homebrew + Node.js 18+
- **Linux**：Node.js 18+ + `dtach` + `ttyd`（安装流程可以帮你补）
- 至少安装了一个 AI 工具（`claude`、`codex`、`cline` 或兼容的本地工具）
...
| `scripts/chat-instance.sh` → `chat-server.mjs` | `7692` | RemoteLab 自举开发时可随时重启的验证 plane |
| `auth-proxy.mjs` | `7681` | 冻结的应急终端 fallback |
...
- 开发 RemoteLab 自身时，`7690` 是稳定 operator plane，`7692` 是重启频繁的 validation plane
