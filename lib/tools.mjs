...
async function ensureDefaultMicroAgentToolRegistrationLoaded(customTools, sharedTools = []) {
  const loadedCustomTools = Array.isArray(customTools) ? customTools : [];
  if (customToolsCacheMtimeMs !== null) {
    return loadedCustomTools;
  }
  if (loadedCustomTools.some(isMicroAgentToolRecord) || sharedTools.some(isMicroAgentToolRecord)) {
    return loadedCustomTools;
  }

  const codexAvailable = !!await resolveToolCommandPathAsync('codex');
  if (!codexAvailable) {
    return loadedCustomTools;
  }

  const { record } = await buildMicroAgentToolRecord();
  const nextTools = [...loadedCustomTools, record];
  await saveCustomToolsAsync(nextTools);
  console.log(`[tools] Seeded default Micro Agent preset into ${TOOLS_FILE}`);
  return nextTools;
}
...
function normalizeSimpleReasoning(reasoning, runtimeFamily) {
  ...
  const allowedKinds = new Set(['none', 'enum', 'toggle']);
...
export async function ensureDefaultMicroAgentToolRegistrationAsync() {
  const [customTools, sharedTools] = await Promise.all([
    loadCustomTools(),
    loadSharedTools(),
  ]);
  const ensured = await ensureDefaultMicroAgentToolRegistrationLoaded(customTools, sharedTools);
  return ensured.some(isMicroAgentToolRecord);
}
