...
import { access, chmod, copyFile, mkdir, mkdtemp, readFile, readdir, readlink, rename, rm, stat, writeFile } from 'fs/promises';
...
import { resolveCodexHomeDir } from './codex-home.mjs';
...
import {
  ...
  normalizeBaseUrl,
  pickBestAccessUrl,
} from './instance-access.mjs';
import {
  buildGuestExposedHostname,
  buildGuestRouteKey,
  buildGuestRouteServiceTarget,
  firstHostnameLabel,
  isPersistableGuestRouteRecord,
  mergePersistedGuestRouteRegistry,
  normalizeGuestRouteRecord,
  normalizeRouteHost,
} from './guest-instance-routes.mjs';
import {
  writeGuestCodexAuthFile,
  writeGuestCodexConfigFile,
} from './codex-auth-redaction.mjs';
import { writeJsonAtomic } from '../chat/fs-utils.mjs';
import { readPrimaryAccessToken, SYSTEM_IDENTITY_ID } from './auth-config.mjs';
import { queryUsageLedger } from '../chat/usage-ledger.mjs';
import { buildInstanceVersionState, normalizeBuildSummary } from './build-sync.mjs';
import { buildInstanceRuntimeCellEnvironment } from './instance-runtime-cell.mjs';
...
const OWNER_CONFIG_DIR = resolveOwnerConfigDir(HOME_DIR);
...
const GUEST_ROUTE_REGISTRY_FILE = join(OWNER_CONFIG_DIR, 'guest-instance-routes.json');
const CLOUDFLARED_CONFIG_FILE = join(HOME_DIR, '.cloudflared', 'config.yml');
const LINUX_CLOUDFLARED_CONFIG_FILE = '/etc/cloudflared/config.yml';
const LINUX_GUEST_ENV_DIR = '/etc/remotelab/guest-instances';
const LINUX_GUEST_SYSTEMD_TEMPLATE = '/etc/systemd/system/remotelab-guest@.service';
const LINUX_GUEST_SYSTEMD_OVERRIDE_DIR = '/etc/systemd/system';
const LINUX_OWNER_ENV_FILE = trimEnvString(process.env.REMOTELAB_OWNER_SYSTEMD_ENV_FILE)
  || '/etc/remotelab/remotelab.env';
const LINUX_GUEST_HOMES_ROOT = '/var/lib/remotelab-guests';
const LINUX_LOG_DIR = '/var/log/remotelab';
...
const PLATFORM_SKILL_DEFINITIONS = Object.freeze([
  {
    id: 'session-delegate',
    fileName: 'session-delegate.md',
    title: 'Visible Session Delegation',
    triggers: ['delegate a task', '子任务', '独立会话', 'session-spawn'],
    description: 'Create a visible independent RemoteLab session and return its link for manual progress observation.',
    repoSourcePath: join(PROJECT_ROOT, 'docs', 'platform-skills', 'session-delegate.md'),
  },
  {
    id: 'calendar-write',
    ...
    description: 'Add events to the instance iCal subscription feed via `remotelab agenda`.',
    repoSourcePath: join(PROJECT_ROOT, 'docs', 'platform-skills', 'calendar-write.md'),
  },
  {
    id: 'session-debug',
    ...
    repoSourcePath: join(PROJECT_ROOT, 'docs', 'platform-skills', 'session-debug.md'),
  },
  {
    id: 'stable-static-publish',
    fileName: 'stable-static-publish.md',
    title: 'Static Page Publish',
    triggers: ['网页预览', '静态网页', 'demo 链接', '静态部署', 'static site'],
    description: 'Publish static output into instance-local RemoteLab storage; reserve Service Port Forwarding for dynamic services.',
    repoSourcePath: join(PROJECT_ROOT, 'docs', 'platform-skills', 'stable-static-publish.md'),
  },
  {
    id: 'guest-port-expose',
    fileName: 'guest-port-expose.md',
    title: 'Service Port Forwarding',
    triggers: [
      '动态服务',
      '内网转发',
      '服务域名',
    ...
    description: 'Forward a loopback-only service through the deployment\'s existing RemoteLab ingress shape; use the managed command for guest instances.',
    repoSourcePath: join(PROJECT_ROOT, 'docs', 'platform-skills', 'guest-port-expose.md'),
  },
  {
    id: 'feishu-cli',
    fileName: 'feishu-cli.md',
    title: 'Feishu CLI',
    triggers: ['飞书文档', '飞书多维表格', '飞书 Wiki', 'Feishu', 'Lark', 'lark-cli', 'Base', 'Bitable'],
    description: 'Use the instance-owned lark-cli Bot profile directly for authorized Feishu capabilities.',
    repoSourcePath: join(PROJECT_ROOT, 'docs', 'platform-skills', 'feishu-cli.md'),
  },
]);
...
function resolveOwnerConfigDir(homeDir) {
  const explicitConfigDir = trimEnvString(process.env.REMOTELAB_OWNER_CONFIG_DIR)
    || trimEnvString(process.env.REMOTELAB_HOST_CONFIG_DIR);
  if (explicitConfigDir) return resolve(explicitConfigDir);

  const homeConfigDir = join(homeDir, '.config', 'remotelab');
  const instanceRoot = trimEnvString(process.env.REMOTELAB_INSTANCE_ROOT);
  if (instanceRoot && resolve(homeDir) === resolve(instanceRoot)) {
    const linuxOwnerConfigDir = '/root/.config/remotelab';
    if (linuxOwnerConfigDir !== homeConfigDir && existsSync(join(linuxOwnerConfigDir, 'guest-instances.json'))) {
      return linuxOwnerConfigDir;
    }
  }
...
const INSTANCE_LOCAL_ACCESS_BOUNDARY_ENFORCED = envFlagEnabled(
  process.env.REMOTELAB_ENFORCE_INSTANCE_LOCAL_BOUNDARY,
  false,
);
const GUEST_SYSTEMD_ISOLATION_ENABLED = process.platform !== 'darwin'
  && envFlagEnabled(
    process.env.REMOTELAB_ENABLE_GUEST_SYSTEMD_ISOLATION,
...
function rewriteHomeRelativePathEntries(pathValue = '', sourceHome = HOME_DIR, targetHome = '') {
  const normalizedSourceHome = trimString(sourceHome);
  const normalizedTargetHome = trimString(targetHome);
  const rewrittenEntries = String(pathValue || '')
    .split(delimiter)
    .map((entry) => trimString(entry))
    .filter(Boolean)
    .map((entry) => {
      if (!normalizedSourceHome || !normalizedTargetHome) return entry;
      // Avoid double-rewriting when the target home itself lives under the source home
      // (for example /root -> /root/.remotelab/instances/trial67 on Linux legacy guests).
      if (entry === normalizedTargetHome || entry.startsWith(`${normalizedTargetHome}/`)) {
        return entry;
      }
      if (entry === normalizedSourceHome) return normalizedTargetHome;
      if (entry.startsWith(`${normalizedSourceHome}/`)) {
        return `${normalizedTargetHome}${entry.slice(normalizedSourceHome.length)}`;
      }
      return entry;
    });
  return dedupePathEntries(rewrittenEntries).join(delimiter);
}

function rewriteLegacyGuestInstancePathEntries(pathValue = '', instanceName = '', targetHome = '') {
  const normalizedInstanceName = trimString(instanceName);
  const normalizedTargetHome = trimString(targetHome);
  if (!normalizedTargetHome) {
    return trimString(pathValue);
  }
  const markerPattern = normalizedInstanceName
    ? new RegExp(`([\\\\/])\\.remotelab\\1instances\\1${escapeRegex(normalizedInstanceName)}(?=$|[\\\\/])`)
    : /([\\/])\.remotelab\1instances\1[^\\/]+(?=$|[\\/])/;
  const rewrittenEntries = String(pathValue || '')
    .split(delimiter)
    .map((entry) => trimString(entry))
    .filter(Boolean)
    .map((entry) => {
      const match = entry.match(markerPattern);
      if (!match) return entry;
      const markerIndex = entry.indexOf(match[0]);
      if (markerIndex < 0) return entry;
      const tail = entry.slice(markerIndex + match[0].length);
      return `${normalizedTargetHome}${tail}`;
    });
  return dedupePathEntries(rewrittenEntries).join(delimiter);
}

function buildGuestPathValue({ homeDir = '', basePath = process.env.PATH || '' } = {}) {
  const normalizedHome = trimString(homeDir) || HOME_DIR;
  const normalizedBasePath = trimString(basePath) || trimString(process.env.PATH) || '';
  const instanceName = basename(normalizedHome);
  const legacyGuestRoot = normalizedHome && trimString(DEFAULT_GUEST_INSTANCES_ROOT)
    ? join(trimString(DEFAULT_GUEST_INSTANCES_ROOT), instanceName)
    : '';
  const legacyRewritten = legacyGuestRoot
    ? rewriteHomeRelativePathEntries(normalizedBasePath, legacyGuestRoot, normalizedHome)
    : normalizedBasePath;
  const legacyInstanceRewritten = rewriteLegacyGuestInstancePathEntries(
    legacyRewritten,
    instanceName,
    normalizedHome,
  );
  const nestedInstanceRewritten = rewriteLegacyGuestInstancePathEntries(
    legacyInstanceRewritten,
    '',
    normalizedHome,
  );
  const rewritten = rewriteHomeRelativePathEntries(nestedInstanceRewritten, HOME_DIR, normalizedHome);
  return trimString(rewritten) || trimString(basePath) || '/usr/local/bin:/usr/bin:/bin';
}

function buildGuestShellEnvSnapshot({
  homeDir = '',
  userName = '',
  ...
  const normalizedHome = trimString(homeDir) || HOME_DIR;
  const normalizedUser = trimString(userName);
  const shellPath = trimString(process.env.REMOTELAB_USER_SHELL)
    || trimString(process.env.SHELL)
    || (process.platform === 'darwin' ? '/bin/zsh' : '/bin/bash');
  const env = {
    SHELL: shellPath,
    HOME: normalizedHome,
    LANG: trimString(process.env.LANG) || 'C.UTF-8',
    PATH: buildGuestPathValue({
      homeDir: normalizedHome,
      basePath,
    }),
    PWD: trimString(workingDirectory) || PROJECT_ROOT,
  };
  if (normalizedUser) {
    env.USER = normalizedUser;
    env.LOGNAME = normalizedUser;
  }
...
function printHelp(stdout = process.stdout) {
  stdout.write([
    ...
    '',
    'Commands:',
    '  create <name>            Create and start a guest instance',
    '  create-trial             Create and start the next standard trial instance',
    '  list                     List guest instances created by this tool',
    '  expose <name>            Publish a guest-owned local port under a controlled hostname',
    '  unexpose <name>          Remove one controlled public hostname for a guest port',
    '  links [name]             Print shareable access links for one or all guest instances',
    '  check [name]             Probe owner + guest build and route consistency',
    '  report [name]            Generate a structured usage + links report',
    '  audit-migration [name]   Compare source-vs-target instance contents before retiring an old host',
    '  show <name>              Show one guest instance and its current access URL',
    '  converge <name>          Repoint one guest instance to the current source tree',
    '  converge --all           Repoint every guest instance to the current source tree',
    '',
    'Create options:',
    '  --trial                  Auto-pick the next `trialN` name when <name> is omitted',
    '  --count <n>             Create multiple instances in one run (batch mode)',
    ...
    '  --isolated               Legacy flag; Linux guests now isolate by default when supported',
    ...
    '',
    'Expose options:',
    '  --label <label>          Route label appended to the guest hostname (required)',
    '  --port <port>            Loopback-only local port to publish (required for expose)',
    '',
    'Converge options:',
    ...
    '',
    'Migration audit options:',
    '  --source <path>          Source instance root directory or downloaded .tgz/.tar.gz archive',
    '  --instance-root <path>   Explicit target instance root override (otherwise resolve from guest registry)',
    '',
    'General options:',
    ...
    '  remotelab guest-instance create-trial --count 5 --json',
    '  remotelab guest-instance create --trial --json',
    '  remotelab guest-instance create trial4',
    '  remotelab guest-instance create share --count 3 --domain example.com',
    '  remotelab guest-instance create alice --isolated --json',
    ...
    '  remotelab guest-instance check --json',
    '  remotelab guest-instance report --json',
    '  remotelab guest-instance report --send',
    '  remotelab guest-instance audit-migration trial4 --source /tmp/migrate-trial4.tgz --json',
...
function parseArgs(argv = []) {
  ...
  const nameCommands = new Set(['create', 'show', 'status', 'links', 'check', 'report', 'converge', 'audit-migration', 'expose', 'unexpose']);
  let startIndex = nameCommands.has(options.command)
    ? (trimString(options.name) ? 2 : 1)
    : 1;
  if (nameCommands.has(options.command) && options.name.startsWith('-')) {
    options.name = '';
    startIndex = 1;
  }
  ...
  for (let index = startIndex; index < argv.length; index += 1) {
    ...
    switch (arg) {
      case '--port':
        ...
        break;
      case '--label':
        options.label = trimString(argv[index + 1]);
        index += 1;
        break;
      case '--hostname':
      ...
      case '--output-dir':
        ...
        break;
      case '--source':
        options.source = trimString(argv[index + 1]);
        index += 1;
        break;
      case '--send':
        options.send = true;
  ...
  if (options.command !== 'create' && options.count !== 1) {
    throw new Error('--count is only supported by create');
  }
  ...
  if (!new Set(['expose', 'unexpose']).has(options.command) && trimString(options.label)) {
    throw new Error('--label is only supported by expose and unexpose');
  }
  if (options.command === 'expose' && !sanitizeGuestInstanceName(options.name)) {
    throw new Error('expose requires <name>');
  }
  if (options.command === 'expose' && !sanitizeGuestInstanceName(options.label)) {
    throw new Error('expose requires --label <label>');
  }
  if (options.command === 'expose' && !(Number.isInteger(options.port) && options.port > 0)) {
    throw new Error('expose requires --port <port>');
  }
  if (options.command === 'unexpose' && !sanitizeGuestInstanceName(options.name)) {
    throw new Error('unexpose requires <name>');
  }
  if (options.command === 'unexpose' && !sanitizeGuestInstanceName(options.label)) {
    throw new Error('unexpose requires --label <label>');
  }
  if (options.command === 'unexpose' && options.port !== null) {
    throw new Error('--port is only supported by expose');
  }
  if (options.command === 'create' && options.count > 1) {
    if (options.port !== null) {
      throw new Error('--port is only supported for single-instance create');
    }
    if (trimString(options.hostname)) {
      throw new Error('--hostname is only supported for single-instance create');
    }
    if (trimString(options.subdomain)) {
      throw new Error('--subdomain is only supported for single-instance create');
    }
    if (trimString(options.instanceRoot)) {
      throw new Error('--instance-root is only supported for single-instance create');
    }
    if (!options.trial && !sanitizeGuestInstanceName(options.name)) {
      throw new Error('--count requires <name> or --trial');
    }
  }
  if (options.command !== 'report' && options.outputDir) {
    throw new Error('--output-dir is only supported by report');
  }
  if (options.command !== 'audit-migration' && trimString(options.source)) {
    throw new Error('--source is only supported by audit-migration');
  }
  ...
  if (options.command === 'audit-migration' && !trimString(options.source)) {
    throw new Error('audit-migration requires --source <path>');
  }
  if (options.command === 'audit-migration' && !sanitizeGuestInstanceName(options.name) && !trimString(options.instanceRoot)) {
    throw new Error('audit-migration requires <name> or --instance-root <path>');
  }

  return options;
}

function planGuestInstanceCreateNames(options, { registry = [], isolatedAgents = [] } = {}) {
  ...
  const occupiedNames = new Set(collectExistingGuestNames({ registry, isolatedAgents }));
  const plannedNames = [];

  if (options?.trial === true) {
    while (plannedNames.length < count) {
      const nextName = pickNextTrialInstanceName([...occupiedNames]);
      occupiedNames.add(nextName);
      plannedNames.push(nextName);
    }
    return plannedNames;
  }

  const prefix = sanitizeGuestInstanceName(options?.name);
  if (!prefix) return [];
  ...
  while (plannedNames.length < count) {
    ...
    suffix += 1;
    if (occupiedNames.has(nextName)) continue;
    occupiedNames.add(nextName);
    plannedNames.push(nextName);
  }
...
async function loadGuestInstanceDefaults() {
  return loadInstanceAccessDefaults({
...
export async function syncGuestPlatformSkills(memoryDir, options = {}) {
  ...
  // Skills must live under the guest home represented by memoryDir. Converge is
  // normally run by the owner, so HOME_DIR would otherwise seed /root while the
  // generated guest index points at the guest's own ~/.remotelab path.
  const homeDir = trimString(options.homeDir) || dirname(normalizedMemoryDir);
  ...
  for (const entry of entries) {
    ...
    const repoExists = trimString(entry.repoSourcePath) ? await pathExists(entry.repoSourcePath) : false;
    const seedSourcePath = repoExists ? entry.repoSourcePath : '';
    const shouldSeed = !platformExists && trimString(seedSourcePath);
    let shouldUpdate = false;
    if (platformExists && seedSourcePath) {
      const [platformContent, sourceContent] = await Promise.all([
        readFile(entry.platformPath),
        readFile(seedSourcePath),
      ]);
      shouldUpdate = !platformContent.equals(sourceContent);
    }
    if (shouldSeed) seededSkillIds.push(entry.id);
    if (shouldUpdate) updatedSkillIds.push(entry.id);
    if (!dryRun && (shouldSeed || shouldUpdate)) {
      await copyFile(seedSourcePath, entry.platformPath);
    }
  ...
  return {
    changed: seededSkillIds.length > 0 || updatedSkillIds.length > 0 || indexChanged,
    indexChanged,
    seededSkillIds,
    updatedSkillIds,
...
async function readOwnerFileAssetEnvironment() {
  ...
  if (await pathExists(LINUX_OWNER_ENV_FILE)) {
    const content = await readFile(LINUX_OWNER_ENV_FILE, 'utf8').catch(() => '');
    Object.assign(ownerEnvironmentVariables, parseEnvFile(content));
  }
  return {
...
async function readCloudflaredContent() {
  const candidates = [...new Set([
    CLOUDFLARED_CONFIG_FILE,
    LINUX_CLOUDFLARED_CONFIG_FILE,
  ].filter(Boolean))];
...
function buildLinuxGuestServiceName(name) {
  const normalizedName = sanitizeGuestInstanceName(name);
  if (!normalizedName) return '';
  return `remotelab-guest@${normalizedName}.service`;
}

function buildLinuxGuestEnvFile(name) {
  return join(LINUX_GUEST_ENV_DIR, `${sanitizeGuestInstanceName(name)}.env`);
}

function serializeEnvFile(environmentVariables = {}) {
  const lines = Object.entries(environmentVariables)
...
function parseEnvFile(content = '') {
  ...
  for (const line of String(content || '').split('\n')) {
    ...
    const value = trimString(trimmed.slice(separatorIndex + 1));
...
async function writeLinuxGuestEnvironmentFile(name, environmentVariables = {}, { restrictPermissions = true } = {}) {
  const envFile = buildLinuxGuestEnvFile(name);
  await mkdir(dirname(envFile), { recursive: true });
  await writeTextAtomic(envFile, serializeEnvFile(environmentVariables));
  if (restrictPermissions) {
    // systemd reads EnvironmentFile= as PID 1 (root) before dropping privileges,
    // so env files can be 600 root:root — the guest user never needs to read them.
    await execOrThrow('chmod', ['600', envFile], `Restricting env file permissions for ${name}`);
    await execOrThrow('chown', ['root:root', envFile], `Setting env file ownership for ${name}`);
  }
  return envFile;
}

// ── Isolation: per-instance Linux user + systemd sandboxing ──

function buildIsolatedUserName(instanceName) {
  return `remotelab-${sanitizeGuestInstanceName(instanceName)}`;
}

function buildIsolatedHomePath(instanceName) {
  return join(LINUX_GUEST_HOMES_ROOT, sanitizeGuestInstanceName(instanceName));
}

function buildSystemdOverrideDir(instanceName) {
  const serviceName = buildLinuxGuestServiceName(instanceName);
  return join(LINUX_GUEST_SYSTEMD_OVERRIDE_DIR, `${serviceName}.d`);
}

async function createIsolatedUser(instanceName) {
  const userName = buildIsolatedUserName(instanceName);
  const homePath = buildIsolatedHomePath(instanceName);

  // Create system user with dedicated home directory and no login shell
  await execOrThrow('/usr/sbin/useradd', [
    '--system',
    '--home-dir', homePath,
    '--create-home',
    '--shell', '/usr/sbin/nologin',
    userName,
  ], `Creating isolated user ${userName}`);

  // Ensure the parent directory is not listable by other users (can traverse, can't list)
  await execOrThrow('chmod', ['711', LINUX_GUEST_HOMES_ROOT],
    `Restricting guest homes root listing`);

  // Lock down the user's home to owner-only
  await execOrThrow('chmod', ['700', homePath],
    `Restricting home directory for ${userName}`);

  return { userName, homePath };
}

async function seedGuestToolCredentials(homePath, { userName = '' } = {}) {
  const normalizedHomePath = trimString(homePath);
  if (!normalizedHomePath) return;

  // Clone Claude Code credentials if available
  const ownerClaudeDir = join(HOME_DIR, '.claude');
  const guestClaudeDir = join(normalizedHomePath, '.claude');
  const ownerCredentials = join(ownerClaudeDir, '.credentials.json');
  if (await pathExists(ownerCredentials)) {
    await mkdir(guestClaudeDir, { recursive: true });
    await copyFile(ownerCredentials, join(guestClaudeDir, '.credentials.json'));
  }

  // Clone Codex credentials if available
  const ownerCodexDir = join(HOME_DIR, '.codex');
  const guestCodexDir = join(normalizedHomePath, '.codex');
  const ownerCodexAuth = join(ownerCodexDir, 'auth.json');
  await mkdir(guestCodexDir, { recursive: true });
  await writeGuestCodexConfigFile(join(guestCodexDir, 'config.toml'));
  if (await pathExists(ownerCodexAuth)) {
    await writeGuestCodexAuthFile({
      sourcePath: ownerCodexAuth,
      targetPath: join(guestCodexDir, 'auth.json'),
    });
  }

  if (trimString(userName)) {
    // Set ownership of everything under the guest home to the isolated user.
    await execOrThrow('chown', ['-R', `${userName}:${userName}`, normalizedHomePath],
      `Setting ownership for ${normalizedHomePath}`);
  }
}

function buildIsolationOverrideContent(instanceName) {
  const userName = buildIsolatedUserName(instanceName);
  const homePath = buildIsolatedHomePath(instanceName);
  return [
...
async function writeIsolationOverride(instanceName) {
  const overrideDir = buildSystemdOverrideDir(instanceName);
  await mkdir(overrideDir, { recursive: true });
  const overridePath = join(overrideDir, 'isolation.conf');
  await writeTextAtomic(overridePath, buildIsolationOverrideContent(instanceName));
  return overridePath;
}

async function removeIsolationOverride(instanceName) {
  const overrideDir = buildSystemdOverrideDir(instanceName);
  const overridePath = join(overrideDir, 'isolation.conf');
  try {
...
async function provisionIsolatedInstance(instanceName) {
  const { userName, homePath } = await createIsolatedUser(instanceName);
  await seedGuestToolCredentials(homePath, { userName });
  const overridePath = await writeIsolationOverride(instanceName);
  return { userName, homePath, overridePath };
}

async function startLinuxGuestService(name) {
  if (!await pathExists(LINUX_GUEST_SYSTEMD_TEMPLATE)) {
    throw new Error(`Missing Linux guest systemd template: ${LINUX_GUEST_SYSTEMD_TEMPLATE}`);
  }
  const serviceName = buildLinuxGuestServiceName(name);
  if (!serviceName) {
    throw new Error('Could not determine guest service name');
  }
  await execOrThrow('systemctl', ['daemon-reload'], 'Reloading systemd');
  await execOrThrow('systemctl', ['enable', '--now', serviceName], `Starting ${serviceName}`);
  return serviceName;
}

async function restartLinuxGuestService(name, { reloadDefinition = false } = {}) {
  const serviceName = buildLinuxGuestServiceName(name);
  if (!serviceName) {
    throw new Error('Could not determine guest service name');
  }
  ...
  await execOrThrow('systemctl', ['restart', serviceName], `Restarting ${serviceName}`);
  return serviceName;
}
...
function normalizeGuestInstanceRecord(record = {}) {
  ...
  // Preserve isolation metadata when present (omit when not isolated for backward compatibility)
  const isolatedUser = trimString(record.isolatedUser);
  if (isolatedUser) {
    normalized.isolatedUser = isolatedUser;
  }
  const mailboxAddress = trimString(record.mailboxAddress);
  if (mailboxAddress) {
...
function isPersistableGuestRegistryRecord(record = {}) {
  if (!trimString(record?.name)) return false;
  if ((Number.parseInt(record?.port, 10) || 0) > 0) return true;
  return Boolean(
    trimString(record?.hostname)
    || trimString(record?.publicBaseUrl)
    || trimString(record?.localBaseUrl)
    || trimString(record?.authFile)
    || trimString(record?.mailboxAddress)
  );
}

function isManagedGuestInstanceRecord(record = {}) {
  return Boolean(trimString(record?.name) && (Number.parseInt(record?.port, 10) || 0) > 0);
}

function mergePersistedGuestRegistry(existingRecords = [], nextRecords = []) {
  const normalizedExisting = Array.isArray(existingRecords)
    ? existingRecords.map((record) => normalizeGuestInstanceRecord(record)).filter((record) => isPersistableGuestRegistryRecord(record))
    : [];
  const normalizedNext = Array.isArray(nextRecords)
    ? nextRecords.map((record) => normalizeGuestInstanceRecord(record)).filter((record) => isPersistableGuestRegistryRecord(record))
    : [];
  const nextByName = new Map(normalizedNext.map((record) => [record.name, record]));
  return [
    ...normalizedNext,
    ...normalizedExisting.filter((record) => !nextByName.has(record.name)),
...
function buildGuestSafeOutboundConfig(ownerOutboundConfig = {}) {
  const provider = trimString(ownerOutboundConfig?.provider) || 'resend_api';
  const workerTokenEnv = trimString(ownerOutboundConfig?.workerTokenEnv)
    || (provider === 'cloudflare_worker' ? 'REMOTELAB_CLOUDFLARE_EMAIL_WORKER_TOKEN' : '');
  const apiKeyEnv = trimString(ownerOutboundConfig?.apiKeyEnv)
    || (provider === 'resend_api' ? 'RESEND_API_KEY' : '');
  const apiBaseUrl = trimString(ownerOutboundConfig?.apiBaseUrl)
    || (provider === 'resend_api' ? 'https://api.resend.com' : '');
  return {
    provider,
    workerBaseUrl: trimString(ownerOutboundConfig?.workerBaseUrl),
    account: '',
    from: '',
    workerToken: '',
    workerTokenEnv,
...
function buildGuestSafeMailboxAutomation(ownerAutomation = {}, record = {}) {
  return {
    ...ownerAutomation,
    chatBaseUrl: trimString(record?.localBaseUrl) || trimString(ownerAutomation?.chatBaseUrl),
    authFile: trimString(record?.authFile) || trimString(ownerAutomation?.authFile),
    deliveryMode: 'session_only',
  };
}

async function syncGuestMailboxPrivacyBaseline(record, ownerMailboxRoot = DEFAULT_ROOT_DIR) {
  const mailboxRoot = trimString(record?.mailboxRoot) || join(trimString(record?.configDir), 'agent-mailbox');
  if (!mailboxRoot || !await pathExists(mailboxRoot)) {
    return {
      changed: false,
    };
  }

  const [ownerOutboundConfig, ownerAutomation, currentOutboundConfig, currentAutomation] = await Promise.all([
    loadOutboundConfig(ownerMailboxRoot),
    loadMailboxAutomation(ownerMailboxRoot),
    loadOutboundConfig(mailboxRoot),
    loadMailboxAutomation(mailboxRoot),
  ]);

  const nextOutboundConfig = buildGuestSafeOutboundConfig(ownerOutboundConfig);
  const nextAutomation = buildGuestSafeMailboxAutomation(ownerAutomation, record);
  const outboundChanged = JSON.stringify(currentOutboundConfig) !== JSON.stringify(nextOutboundConfig);
  const automationChanged = JSON.stringify(currentAutomation) !== JSON.stringify(nextAutomation);

  if (outboundChanged) {
    await saveOutboundConfig(mailboxRoot, nextOutboundConfig);
  }
  if (automationChanged) {
    await saveMailboxAutomation(mailboxRoot, nextAutomation);
  }

  return {
    changed: outboundChanged || automationChanged,
    outboundChanged,
    automationChanged,
  };
}
...
async function provisionGuestMailbox(record, ownerMailboxRoot = DEFAULT_ROOT_DIR) {
  ...
  // Guest instances must not inherit the owner's sender allowlist by default.
  // Copying owner-level allowlists leaks private sender addresses into public services.
  await saveAllowlist(mailboxRoot, {
    allowedEmails: [],
    allowedDomains: [],
  });
  await saveOutboundConfig(mailboxRoot, buildGuestSafeOutboundConfig(await loadOutboundConfig(ownerMailboxRoot)));
  const ownerAutomation = await loadMailboxAutomation(ownerMailboxRoot);
  await saveMailboxAutomation(mailboxRoot, buildGuestSafeMailboxAutomation(ownerAutomation, record));
...
async function loadGuestRegistry() {
  ...
  return records
    ...
    .filter((record) => isManagedGuestInstanceRecord(record));
}

async function saveGuestRegistry(records) {
  const existingRecords = await readJsonFile(GUEST_REGISTRY_FILE, []);
  const normalizedRecords = mergePersistedGuestRegistry(existingRecords, records);
  await writeJsonAtomic(GUEST_REGISTRY_FILE, normalizedRecords);
}

async function loadGuestRouteRegistry() {
  const records = await readJsonFile(GUEST_ROUTE_REGISTRY_FILE, []);
  if (!Array.isArray(records)) return [];
  return records
    .map((record) => normalizeGuestRouteRecord(record))
    .filter((record) => isPersistableGuestRouteRecord(record));
}

async function saveGuestRouteRegistry(records) {
  const normalizedRecords = Array.isArray(records)
    ? records.map((record) => normalizeGuestRouteRecord(record)).filter((record) => isPersistableGuestRouteRecord(record))
    : [];
  await writeJsonAtomic(GUEST_ROUTE_REGISTRY_FILE, normalizedRecords);
}

function attachExposedRoutes(record, routes = []) {
  const matchingRoutes = routes
    .filter((route) => route.instanceName === record.name)
    .sort((leftRoute, rightRoute) => leftRoute.label.localeCompare(rightRoute.label, undefined, {
      numeric: true,
      sensitivity: 'base',
    }))
    .map((route) => ({
      ...route,
      publicBaseUrl: `https://${route.hostname}`,
      serviceTarget: buildGuestRouteServiceTarget(route),
    }));
  if (matchingRoutes.length === 0) return { ...record, exposedRoutes: [] };
  return {
    ...record,
    exposedRoutes: matchingRoutes,
  };
}

function normalizeListenerAddress(value) {
  return trimString(String(value || '').replace(/\s+\(listen\)\s*$/i, ''));
}

function isLoopbackListenerAddress(value, port) {
  const normalizedAddress = normalizeListenerAddress(value);
  const expectedPort = `:${Number.parseInt(String(port || ''), 10) || 0}`;
  if (!normalizedAddress || !normalizedAddress.endsWith(expectedPort)) return false;
  const host = normalizeListenerAddress(normalizedAddress.slice(0, -expectedPort.length))
    .replace(/^\[|\]$/g, '')
    .toLowerCase();
  return host === '127.0.0.1' || host === 'localhost' || host === '::1';
}

function parseLsofProcessRecords(content = '') {
  const records = [];
  let current = null;
  for (const line of String(content || '').split('\n')) {
    if (!line) continue;
    ...
    if (!current) continue;
    if (fieldType === 'L') {
      current.loginName = trimString(fieldValue);
      continue;
    }
    if (fieldType === 'c') {
      current.command = trimString(fieldValue);
      continue;
    }
    if (fieldType === 'n') {
      current.addresses.push(normalizeListenerAddress(fieldValue));
    }
  }
  if (current?.pid) records.push(current);
  return records;
...
async function inspectListeningPort(port, { execFileFn = execFileAsync } = {}) {
  try {
    const result = await execFileFn('lsof', ['-nP', `-iTCP:${port}`, '-sTCP:LISTEN', '-FpLcn'], {
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'ignore'],
    });
    const listeners = parseLsofProcessRecords(result?.stdout || '');
    const loginNames = [...new Set(listeners.map((entry) => trimString(entry.loginName)).filter(Boolean))];
    return {
      port,
      listening: listeners.length > 0,
      listeners,
      loginNames,
      loopbackOnly: listeners.length > 0
        && listeners.every((entry) => entry.addresses.length > 0 && entry.addresses.every((address) => isLoopbackListenerAddress(address, port))),
    };
  } catch (error) {
...
function buildGuestExposedRouteRecord(record, { label = '', port = 0 } = {}) {
  const normalizedLabel = sanitizeGuestInstanceName(label);
  const hostname = buildGuestExposedHostname({
    hostname: record?.hostname,
    publicBaseUrl: record?.publicBaseUrl,
    label: normalizedLabel,
  });
  if (!hostname) {
    throw new Error(`Guest instance ${record?.name || 'guest'} does not have a routable public hostname`);
  }
  return normalizeGuestRouteRecord({
...
function collectGuestRouteLookupConflicts({ registry = [], routes = [], ignoreRouteKey = '' } = {}) {
  const reservedLabels = new Set(HOST_ROUTER_RESERVED_LABELS);
  const usedHostnames = new Map();
  const usedLabels = new Map();

  for (const record of registry) {
    const normalizedHostname = normalizeRouteHost(record?.hostname);
    const hostLabel = firstHostnameLabel(record?.hostname);
    if (normalizedHostname) usedHostnames.set(normalizedHostname, `guest:${record.name}`);
    if (record?.name) usedLabels.set(record.name, `guest:${record.name}`);
    if (hostLabel) usedLabels.set(hostLabel, `guest:${record.name}`);
  }

  for (const route of routes) {
    const routeKey = buildGuestRouteKey(route);
    if (!routeKey || routeKey === ignoreRouteKey) continue;
    const normalizedHostname = normalizeRouteHost(route.hostname);
    const hostLabel = firstHostnameLabel(route.hostname);
    if (normalizedHostname) usedHostnames.set(normalizedHostname, `route:${routeKey}`);
    if (hostLabel) usedLabels.set(hostLabel, `route:${routeKey}`);
  }
...
function buildGuestWeChatConnectorConfig({ localBaseUrl = '' } = {}) {
  const chatBaseUrl = normalizeBaseUrl(localBaseUrl);
  return {
    chatBaseUrl,
    sourceName: 'WeChat',
    group: 'WeChat',
...
async function seedGuestWeChatConnectorConfig(configDir, { localBaseUrl = '' } = {}) {
  const connectorDir = join(configDir, 'wechat-connector');
  const configPath = join(connectorDir, 'config.json');
  const exists = await pathExists(configPath);
  if (!exists) {
    await writeJsonAtomic(configPath, buildGuestWeChatConnectorConfig({ localBaseUrl }));
  }
  return {
    changed: !exists,
    configPath,
...
function buildGuestWeChatConnectorEnv({ instanceRoot = '', port = 0, publicBaseUrl = '' } = {}) {
  const env = {
    ...process.env,
  };
  ...
  env.HOME = normalizedInstanceRoot || trimString(env.HOME) || HOME_DIR;
  env.PATH = buildGuestPathValue({
    homeDir: env.HOME,
    basePath: trimString(env.PATH) || process.env.PATH || '',
  });
  if (normalizedInstanceRoot) {
    env.REMOTELAB_INSTANCE_ROOT = normalizedInstanceRoot;
    env.TMPDIR = join(normalizedInstanceRoot, 'tmp');
  }

  const normalizedPort = Number.parseInt(String(port || ''), 10) || 0;
  if (normalizedPort > 0) {
    env.CHAT_PORT = String(normalizedPort);
  }
...
async function startGuestWeChatConnector({ instanceRoot = '', port = 0, publicBaseUrl = '', execFileFn = execFileAsync } = {}) {
  const normalizedInstanceRoot = trimString(instanceRoot);
  const normalizedPort = Number.parseInt(String(port || ''), 10) || 0;
  if (!normalizedInstanceRoot) {
    throw new Error('startGuestWeChatConnector requires instanceRoot');
  }
  if (normalizedPort <= 0) {
    throw new Error('startGuestWeChatConnector requires a valid port');
  }

  const result = await execFileFn(join(PROJECT_ROOT, 'scripts', 'wechat-connector-instance.sh'), ['start'], {
    cwd: PROJECT_ROOT,
    env: buildGuestWeChatConnectorEnv({
      instanceRoot: normalizedInstanceRoot,
      port: normalizedPort,
      publicBaseUrl,
    }),
    encoding: 'utf8',
...
function getAccessUrl(record, token) {
  return pickBestAccessUrl({
    publicBaseUrl: trimString(record?.publicBaseUrl),
    bridgeBaseUrl: trimString(record?.bridgeBaseUrl),
    localBaseUrl: trimString(record?.localBaseUrl),
    token,
  });
}
...
function attachDerivedAccess(record, { token = '', guestDefaults = null } = {}) {
  return attachInstanceAccess(record, {
    token,
    bridgeBaseUrl: buildBridgeBaseUrl(record?.name, guestDefaults),
  });
}
...
async function discoverIsolatedLaunchAgents() {
  const cloudflaredContent = await readCloudflaredContent();
  const hostnamesByPort = buildPortHostnameMap(cloudflaredContent);

  if (process.platform !== 'darwin') {
    const registry = await loadGuestRegistry().catch(() => []);
    const agents = [];
    for (const record of registry) {
      const name = sanitizeGuestInstanceName(record?.name);
      if (!name) continue;
      const launchAgentPath = trimString(record?.launchAgentPath) || buildLinuxGuestEnvFile(name);
      const environmentVariables = parseEnvFile(await readFile(launchAgentPath, 'utf8').catch(() => ''));
      const port = Number.parseInt(
        trimString(environmentVariables.CHAT_PORT) || trimString(record?.port),
        10,
      ) || 0;
      if (port <= 0) continue;
      const hostname = trimString(record?.hostname) || hostnamesByPort.get(port) || '';
      const instanceRoot = trimString(record?.instanceRoot)
        || (trimString(record?.isolatedUser) ? buildIsolatedHomePath(name) : '')
        || trimString(environmentVariables.REMOTELAB_INSTANCE_ROOT)
        || join(DEFAULT_GUEST_INSTANCES_ROOT, name);
      agents.push({
        label: trimString(record?.label) || buildLinuxGuestServiceName(name).replace(/\.service$/, ''),
        name,
        nodePath: '/usr/bin/env',
        chatServerPath: join(PROJECT_ROOT, 'chat-server.mjs'),
        workingDirectory: PROJECT_ROOT,
        standardOutPath: trimString(record?.logPath) || join(LINUX_LOG_DIR, `chat-server-${name}.log`),
        standardErrorPath: trimString(record?.errorLogPath) || join(LINUX_LOG_DIR, `chat-server-${name}.error.log`),
        environmentVariables,
        port,
        instanceRoot,
        isolatedUser: trimString(record?.isolatedUser),
        launchAgentPath,
        hostname,
    ...
    if (agents.length > 0 || !await pathExists(LAUNCH_AGENTS_DIR)) {
      return agents;
    }
  }
...
function buildConvergedLaunchAgentSpec(agent = {}, options = {}) {
  ...
  delete environmentVariables.REMOTELAB_BRIDGE_BASE_URL;
  delete environmentVariables.REMOTELAB_BRIDGE_BASE_URL_TEMPLATE;
  delete environmentVariables.REMOTELAB_BRIDGE_ROOT_BASE_URL;
  delete environmentVariables.REMOTELAB_MACHINE_CODEX_HOME;
  delete environmentVariables.REMOTELAB_CODEX_HOME_MODE;
  delete environmentVariables.REMOTELAB_SHARED_CODEX_HOME;
  delete environmentVariables.REMOTELAB_SHARED_CODEX_LOCK_FILE;
  ...
  const desiredInstanceRoot = trimString(agent.instanceRoot)
    || (trimString(agent?.isolatedUser) && name ? buildIsolatedHomePath(name) : '')
    || trimString(environmentVariables.REMOTELAB_INSTANCE_ROOT)
    || (name ? join(DEFAULT_GUEST_INSTANCES_ROOT, name) : '');
  Object.assign(environmentVariables, buildInstanceRuntimeCellEnvironment({
    instanceRoot: desiredInstanceRoot,
    projectRoot: PROJECT_ROOT,
  }));
  environmentVariables.TMPDIR = trimString(environmentVariables.REMOTELAB_INSTANCE_ROOT)
    ? join(environmentVariables.REMOTELAB_INSTANCE_ROOT, 'tmp')
    : trimString(environmentVariables.TMPDIR);
  const currentInstanceRoot = trimString(currentEnv.REMOTELAB_INSTANCE_ROOT);
  const legacyNestedInstanceMarker = name ? `${sep}.remotelab${sep}instances${sep}${name}${sep}` : '';
  const shouldRebuildPathFromHost = Boolean(
    (currentInstanceRoot && currentInstanceRoot !== desiredInstanceRoot)
    || (legacyNestedInstanceMarker && trimString(environmentVariables.PATH).includes(legacyNestedInstanceMarker)),
  );
  environmentVariables.PATH = buildGuestPathValue({
    homeDir: environmentVariables.HOME,
    basePath: shouldRebuildPathFromHost
      ? (process.env.PATH || '')
      : (trimString(environmentVariables.PATH) || process.env.PATH || ''),
  });
  const inferredGuestUser = trimString(agent?.isolatedUser)
    || (trimString(environmentVariables.HOME).startsWith(`${LINUX_GUEST_HOMES_ROOT}${sep}`)
      ? buildIsolatedUserName(name)
      : '');
  environmentVariables.REMOTELAB_USER_SHELL_ENV_B64 = buildGuestShellEnvSnapshot({
    homeDir: environmentVariables.HOME,
    userName: inferredGuestUser,
    basePath: environmentVariables.PATH,
    workingDirectory: PROJECT_ROOT,
  });
...
function detectLaunchAgentConvergence(agent = {}, next = buildConvergedLaunchAgentSpec(agent)) {
  ...
  const homeChanged = trimString(currentEnv.HOME) !== trimString(next.environmentVariables?.HOME);
  const missingTmpDir = !trimString(currentEnv.TMPDIR);
  const tmpDirChanged = trimString(currentEnv.TMPDIR) !== trimString(next.environmentVariables?.TMPDIR);
  const missingInstanceRoot = !trimString(currentEnv.REMOTELAB_INSTANCE_ROOT);
  const instanceRootChanged = trimString(currentEnv.REMOTELAB_INSTANCE_ROOT)
    !== trimString(next.environmentVariables?.REMOTELAB_INSTANCE_ROOT);
  ...
  const instanceRuntimeEnvironmentChanged = INSTANCE_RUNTIME_CELL_ENV_KEYS.some(
    (key) => trimString(currentEnv[key]) !== trimString(next.environmentVariables?.[key]),
  );
  const obsoleteCodexEnvPresent = [
    currentEnv.REMOTELAB_MACHINE_CODEX_HOME,
    currentEnv.REMOTELAB_CODEX_HOME_MODE,
    currentEnv.REMOTELAB_SHARED_CODEX_HOME,
    currentEnv.REMOTELAB_SHARED_CODEX_LOCK_FILE,
  ].some((value) => trimString(value));
  const shellEnvSnapshotChanged = trimString(currentEnv.REMOTELAB_USER_SHELL_ENV_B64)
    !== trimString(next.environmentVariables?.REMOTELAB_USER_SHELL_ENV_B64);
  const pathChanged = trimString(currentEnv.PATH) !== trimString(next.environmentVariables?.PATH);
  const bridgeEnvPresent = [
    currentEnv.REMOTELAB_BRIDGE_BASE_URL,
    currentEnv.REMOTELAB_BRIDGE_BASE_URL_TEMPLATE,
    currentEnv.REMOTELAB_BRIDGE_ROOT_BASE_URL,
  ].some((value) => trimString(value));
  ...
  return {
    changed: codePathChanged
      ...
      || instanceRuntimeEnvironmentChanged
      || obsoleteCodexEnvPresent
      || shellEnvSnapshotChanged
      || pathChanged
      || bridgeEnvPresent
...
async function detectOwnerCodexModel() {
  const codexConfigPath = join(resolveCodexHomeDir(), 'config.toml');
...
function isLegacyMicroAgentToolRecord(toolId, tool = null) {
  return isLegacyMicroAgentToolId(toolId) || trimString(tool?.id) === LEGACY_MICRO_AGENT_TOOL_ID;
}

function normalizeGuestCopyableOwnerTool(tool) {
  if (!tool || typeof tool !== 'object') return null;
  if (isLegacyMicroAgentToolRecord(tool?.id, tool)) return null;
...
function buildGuestOwnerSelectionCandidate(selection, tool = null) {
  ...
  if (!isLegacyMicroAgentToolRecord(selection.selectedTool, tool)) {
    return selection;
  }
  return {
    ...selection,
...
function resolveToolReasoningKind(toolId, tool = null) {
  ...
  if (normalizedKind === 'enum' || normalizedKind === 'none') {
...
function resolveToolModelId(toolId, tool = null, candidateSelection = null, detectedModel = '') {
  ...
  if (candidateModel && (models.length === 0 || models.some((model) => trimString(model?.id) === candidateModel))) {
    return trimString(toolId) === 'codex' ? normalizeCodexModelId(candidateModel) : candidateModel;
  }

  if (trimString(toolId) === 'codex') {
    return normalizeCodexModelId(detectedModel) || PRODUCT_DEFAULT_CODEX_MODEL;
  }
...
function resolveToolEffort({
  ...
  if (normalizedToolId === PRODUCT_DEFAULT_TOOL_ID && levels.length === 0) {
    return PRODUCT_DEFAULT_CODEX_EFFORT;
  }
...
export function planGuestRuntimeDefaults({
  ...
  const normalizedGuestTools = Array.isArray(guestTools)
    ? guestTools.filter((tool) => tool && typeof tool === 'object')
      .filter((tool) => !isLegacyMicroAgentToolRecord(tool?.id, tool))
    : [];
...
async function syncGuestRuntimeTools(configDir, options = {}) {
...
async function seedGuestRuntimeTools(configDir) {
  await syncGuestRuntimeTools(configDir);
}

async function writeGuestAuthFile(authFile, { token, username = '', password = '' }) {
  const now = new Date().toISOString();
  const personName = trimString(username) || 'Administrator';
  const credentials = [{
  ...
  const nextAuth = {
    ...
    people: [{
      ...
      credentials,
      identities: [{
        id: 'identity_web_default',
        kind: 'web',
        realm: 'remotelab',
        subjectId: 'person_default',
...
async function ensureGuestLayout({ name, hostname, instanceRoot, configDir, memoryDir }) {
  ...
  await mkdir(join(instanceRoot, 'workspace'), { recursive: true });
  await mkdir(join(instanceRoot, 'tmp'), { recursive: true });
...
async function fetchRouteProbeWithCurl(baseUrl, pathname, timeoutMs) {
  const statusMarker = '__REMOTELAB_HTTP_STATUS__:';
  try {
    const result = await execFileAsync('curl', [
      '-sS',
    ...
    return {
      status: 0,
      bodyText: trimString(error?.stderr?.toString?.() || error?.message || String(error)),
    };
  }
}

async function fetchRouteProbe(baseUrl, pathname, timeoutMs) {
  if (shouldUseCurlBuildInfoProbe(baseUrl)) {
    const curlResult = await fetchRouteProbeWithCurl(baseUrl, pathname, timeoutMs);
    if (curlResult) return curlResult;
  }
  ...
  try {
    ...
    return {
      status: 0,
      bodyText: trimString(error?.message || String(error)),
    };
  } finally {
    clearTimeout(timer);
  }
...
function summarizeProtectedRouteProbe(probe = null) {
  const status = Number.parseInt(probe?.status, 10) || 0;
  return {
    status,
    ok: status === 200 || status === 401 || status === 403,
    bodyPreview: trimString(probe?.bodyText || '').slice(0, 160),
  };
}

function buildVersionStateFromProbes({ ownerProbe = null, localProbe = null, publicProbe = null } = {}) {
  return buildInstanceVersionState({
    owner: normalizeBuildSummary(ownerProbe?.body || null),
    local: normalizeBuildSummary(localProbe?.body || null),
    publicBuild: normalizeBuildSummary(publicProbe?.body || null),
  });
}

function usesLaunchAgentRuntime(target = {}) {
  return trimString(target?.launchAgentPath).endsWith('.plist');
}

async function restartGuestService(target, { reloadDefinition = false } = {}) {
  if (usesLaunchAgentRuntime(target)) {
  ...
  await restartLinuxGuestService(target.name, { reloadDefinition });
}

async function runBootstrapBackfill({ instanceRoot, port }) {
  const env = {
    ...process.env,
  };
  delete env.REMOTELAB_CONFIG_DIR;
  delete env.REMOTELAB_MEMORY_DIR;
  env.REMOTELAB_INSTANCE_ROOT = trimString(instanceRoot);
  env.CHAT_PORT = String(Number.parseInt(port, 10) || 0);

  const result = await execFileAsync(
    process.execPath,
    [join(PROJECT_ROOT, 'scripts', 'backfill-bootstrap.mjs')],
    {
      cwd: PROJECT_ROOT,
      env,
      encoding: 'utf8',
...
async function readTokenFromAuthFile(authFile) {
  return readPrimaryAccessToken(authFile).catch(() => '');
}
...
function formatGuestInstance(record, { token = '', localReachable = null, publicReachable = null, warnings = [] } = {}) {
  ...
  if (record.bridgeBaseUrl) {
    lines.push(`bridge: ${record.bridgeBaseUrl}`);
  }
  if (Array.isArray(record.exposedRoutes)) {
    for (const route of record.exposedRoutes) {
      lines.push(`exposedRoute: ${route.label} -> https://${route.hostname} (${route.serviceTarget})`);
    }
  }
  if (token) {
    ...
    if (record.bridgeAccessUrl || record.bridgeBaseUrl) {
      lines.push(`bridgeAccess: ${record.bridgeAccessUrl || buildAccessUrl(record.bridgeBaseUrl, token)}`);
    }
...
function formatGuestInstanceLinks(records = [], { check = false } = {}) {
  ...
  return records.map((record) => {
    ...
    if (record.bridgeAccessUrl) {
      lines.push(`bridgeAccess: ${record.bridgeAccessUrl}`);
    }
    ...
    if (check) {
      ...
      if (record.build?.label) {
        lines.push(`buildStatus: ${record.build.label}`);
      }
    }
    return lines.join('\n');
  }).join('\n\n');
...
function formatGuestExposeResult(result = {}) {
  const lines = [
    `instance: ${result.instanceName}`,
    `label: ${result.label}`,
    `status: ${result.changed ? 'updated' : 'unchanged'}`,
  ];
  if (trimString(result.hostname)) {
    lines.push(`hostname: ${result.hostname}`);
  }
  if (trimString(result.publicBaseUrl)) {
    lines.push(`public: ${result.publicBaseUrl}`);
  }
  if (trimString(result.accessUrl)) {
    lines.push(`access: ${result.accessUrl}`);
  }
  if (trimString(result.serviceTarget)) {
    lines.push(`target: ${result.serviceTarget}`);
  }
  for (const binding of Array.isArray(result.listenerBindings) ? result.listenerBindings : []) {
    lines.push(`listener: ${binding}`);
  }
  return lines.join('\n');
}

function formatGuestUnexposeResult(result = {}) {
  const lines = [
    `instance: ${result.instanceName}`,
    `label: ${result.label}`,
    `status: ${result.changed ? 'removed' : 'not-found'}`,
  ];
  if (trimString(result.hostname)) {
    lines.push(`hostname: ${result.hostname}`);
  }
...
function formatConvergeResults(results = []) {
  ...
  return results.map((result) => {
    ...
    if (result.build?.label) {
      lines.push(`buildStatus: ${result.build.label}`);
    }
    return lines.join('\n');
  }).join('\n\n');
}
...
async function enrichGuestRecord(record, { includeToken = false } = {}) {
  const ownerProbe = await fetchBuildInfo(getLocalBaseUrl(OWNER_PORT), 1500);
  const localProbe = await fetchBuildInfo(record.localBaseUrl, 1500);
  const publicProbe = record.publicBaseUrl
  ...
  const routes = await loadGuestRouteRegistry();
  return attachExposedRoutes(attachDerivedAccess({
    ...record,
    mailboxAddress: buildGuestMailboxAddress(record.name, mailboxIdentity),
    localReachable: localProbe.ok,
    publicReachable: publicProbe ? publicProbe.ok : null,
    build: buildVersionStateFromProbes({
      ownerProbe,
      localProbe,
      publicProbe,
    }),
...
async function listGuestInstanceLinks(options = {}) {
  ...
  const mailboxIdentity = await loadIdentity();
  const routes = await loadGuestRouteRegistry();
  const ownerProbe = options.check
    ? await fetchBuildInfo(getLocalBaseUrl(OWNER_PORT), 1500)
    : null;
  return Promise.all(matchingRecords.map(async (record) => {
    ...
    let build = null;
    if (options.check) {
      [localProbe, publicProbe] = await Promise.all([
      ...
      localReachable = localProbe?.ok === true;
      publicReachable = publicProbe ? publicProbe.ok === true : null;
      build = buildVersionStateFromProbes({
        ownerProbe,
        localProbe,
        publicProbe,
...
function buildFleetCheckIssues({
  localReachable,
  publicReachable,
  build,
  settingsLocal,
  ...
  const issues = [];
  if (localReachable === false) issues.push('local_unreachable');
  if (hasPublicBaseUrl && publicReachable === false) issues.push('public_unreachable');
  if (trimString(build?.status) === 'stale_runtime') issues.push('stale_runtime');
  if (trimString(build?.status) === 'public_mismatch') issues.push('public_build_mismatch');
  if (!settingsLocal?.ok) issues.push(`settings_local_${settingsLocal?.status || 0}`);
  if (hasPublicBaseUrl && settingsPublic && !settingsPublic.ok) issues.push(`settings_public_${settingsPublic.status || 0}`);
  return issues;
}

async function buildFleetCheckRecord(record, { ownerBuildProbe = null, isOwner = false } = {}) {
  const localBuildProbe = await fetchBuildInfo(record.localBaseUrl, 1500);
  const publicBuildProbe = record.publicBaseUrl
    ? await fetchBuildInfo(record.publicBaseUrl, 4000)
    : null;
  const [localSettingsProbe, publicSettingsProbe] = await Promise.all([
    fetchRouteProbe(record.localBaseUrl, '/api/settings', 1500),
    record.publicBaseUrl ? fetchRouteProbe(record.publicBaseUrl, '/api/settings', 4000) : Promise.resolve(null),
  ]);

  const build = buildVersionStateFromProbes({
    ownerProbe: isOwner ? null : ownerBuildProbe,
    localProbe: localBuildProbe,
    publicProbe: publicBuildProbe,
  });
  const settingsLocal = summarizeProtectedRouteProbe(localSettingsProbe);
  const settingsPublic = publicSettingsProbe ? summarizeProtectedRouteProbe(publicSettingsProbe) : null;
  const localReachable = localBuildProbe?.ok === true;
  const publicReachable = publicBuildProbe ? publicBuildProbe.ok === true : null;
  const issues = buildFleetCheckIssues({
    localReachable,
    publicReachable,
    build,
    settingsLocal,
    settingsPublic,
    hasPublicBaseUrl: !!record.publicBaseUrl,
  });

  return {
    name: record.name,
    kind: isOwner ? 'owner' : 'guest',
    port: record.port,
    localBaseUrl: record.localBaseUrl,
    publicBaseUrl: record.publicBaseUrl,
    localReachable,
    publicReachable,
    build,
    routes: {
      buildInfoLocalStatus: Number.parseInt(localBuildProbe?.status, 10) || 0,
      buildInfoPublicStatus: publicBuildProbe ? (Number.parseInt(publicBuildProbe.status, 10) || 0) : null,
      settingsLocalStatus: settingsLocal.status,
      settingsPublicStatus: settingsPublic ? settingsPublic.status : null,
    },
    issues,
    ok: issues.length === 0,
  };
...
function formatGuestInstanceCheckResult(result = {}) {
  const summary = result.summary || {};
  const lines = [
    `generatedAt: ${result.generatedAt || ''}`,
    `ownerServiceAssetVersion: ${result.ownerServiceAssetVersion || '(unknown)'}`,
    `instances: ${summary.totalCount || 0}`,
    `issueCount: ${summary.issueCount || 0}`,
    `buildDrift: ${summary.buildDriftCount || 0}`,
    `publicBuildMismatch: ${summary.publicBuildMismatchCount || 0}`,
    `settingsRouteIssues: ${summary.settingsRouteIssueCount || 0}`,
    '',
  ];

  for (const instance of Array.isArray(result.instances) ? result.instances : []) {
    lines.push([
      `${instance.name}${instance.kind === 'owner' ? ' [owner]' : ''}`,
      `build=${instance.build?.label || '待确认'}`,
      `settings=${instance.routes?.settingsLocalStatus || 0}/${instance.routes?.settingsPublicStatus ?? '-'}`,
      `issues=${instance.issues?.length ? instance.issues.join(',') : 'none'}`,
    ].join(' | '));
  }
...
async function checkGuestInstanceFleet(options = {}) {
  const normalizedName = sanitizeGuestInstanceName(options?.name);
  const registry = await loadGuestRegistry();
  if (normalizedName && !registry.some((record) => record.name === normalizedName)) {
    throw new Error(`Guest instance not found: ${normalizedName}`);
  }

  const hostnamesByPort = buildPortHostnameMap(await readCloudflaredContent());
  const ownerRecord = {
    name: 'owner',
    port: OWNER_PORT,
    localBaseUrl: getLocalBaseUrl(OWNER_PORT),
    publicBaseUrl: trimString(process.env.REMOTELAB_PUBLIC_BASE_URL) || getPublicBaseUrl(hostnamesByPort.get(OWNER_PORT) || ''),
  };
  const ownerBuildProbe = await fetchBuildInfo(ownerRecord.localBaseUrl, 1500);
  const ownerCheckedRecord = await buildFleetCheckRecord(ownerRecord, {
    ownerBuildProbe,
    isOwner: true,
  });

  const guestRecords = (normalizedName
    ? registry.filter((record) => record.name === normalizedName)
    : registry
  ).map((record) => ({
    name: record.name,
  ...
  for (const record of guestRecords) {
    checkedGuests.push(await buildFleetCheckRecord(record, { ownerBuildProbe }));
  }

  const instances = normalizedName
    ? checkedGuests
    : [ownerCheckedRecord, ...checkedGuests];
  const summary = {
    totalCount: instances.length,
    issueCount: instances.filter((instance) => !instance.ok).length,
    buildDriftCount: instances.filter((instance) => trimString(instance.build?.status) === 'stale_runtime').length,
    publicBuildMismatchCount: instances.filter((instance) => trimString(instance.build?.status) === 'public_mismatch').length,
    settingsRouteIssueCount: instances.filter((instance) => instance.issues.some((issue) => issue.startsWith('settings_'))).length,
    localReachabilityIssueCount: instances.filter((instance) => instance.localReachable === false).length,
    publicReachabilityIssueCount: instances.filter((instance) => instance.publicReachable === false).length,
    issueNames: instances.filter((instance) => !instance.ok).map((instance) => instance.name),
  };

  return {
    generatedAt: new Date().toISOString(),
    ownerServiceAssetVersion: trimString(ownerBuildProbe?.body?.serviceAssetVersion),
    instances,
    summary,
  };
}

const MIGRATION_AUDIT_MAX_SAMPLE_ITEMS = 20;
const MIGRATION_AUDIT_PATH_REFERENCE_PATTERN = /\/(?:Users|home|root|opt|etc|var|tmp|srv|mnt|Volumes)[^"'`\s<>()\[\],;:]*/g;
const MIGRATION_AUDIT_TEXT_EXTENSIONS = new Set([
  '.c',
  '.cc',
  '.cfg',
  '.conf',
...
const MIGRATION_AUDIT_CODE_EXTENSIONS = new Set([
  '.c',
  '.cc',
  '.cpp',
  '.css',
...
const MIGRATION_AUDIT_EPHEMERAL_PATH_PATTERNS = Object.freeze([
  /^\/tmp\/[^/]*\.XXXXXX(?:\/.*)?$/,
  /^\/tmp\/class_report(?:\/.*)?$/,
  /^\/var\/folders\/[^/]+\/[^/]+\/T\/remotelab-[^/]+(?:\/.*)?$/,
]);
const MIGRATION_AUDIT_EXPECTED_DRIFT_PATH_PATTERNS = Object.freeze([
  /^config\/chat-history\/[^/]+\/(?:fork-context|meta)\.json$/,
  /^config\/ui-runtime-selection\.json$/,
]);
const MIGRATION_AUDIT_LEGACY_REMOTELAB_REPO_PATTERN = /^\/(?:Users\/[^/]+|home\/[^/]+|root)\/code\/remotelab(?:\/(.*))?$/;

function buildMigrationAuditArchiveArgs(sourcePath, extractionRoot) {
  const lower = trimString(sourcePath).toLowerCase();
  if (lower.endsWith('.tgz') || lower.endsWith('.tar.gz')) {
    return ['-xzf', sourcePath, '-C', extractionRoot];
  }
  if (lower.endsWith('.tar')) {
    return ['-xf', sourcePath, '-C', extractionRoot];
  }
  throw new Error(`Unsupported migration audit source archive: ${sourcePath}`);
}

async function resolveMigrationAuditSource(sourcePath) {
  const normalizedSourcePath = trimString(sourcePath);
  if (!normalizedSourcePath) {
    throw new Error('Migration audit requires a source directory or .tgz archive');
  }

  const resolvedSourcePath = resolve(normalizedSourcePath);
  const sourceStat = await stat(resolvedSourcePath).catch(() => null);
  if (!sourceStat) {
    throw new Error(`Migration audit source not found: ${resolvedSourcePath}`);
  }
  ...
  if (sourceStat.isDirectory()) {
    return {
      ...
      kind: 'directory',
      root: resolvedSourcePath,
      cleanup: async () => {},
    };
  }

  const extractionRoot = await mkdtemp(join(tmpdir(), 'remotelab-guest-audit-'));
  try {
    await execOrThrow('tar', buildMigrationAuditArchiveArgs(resolvedSourcePath, extractionRoot), `Extracting migration audit source ${resolvedSourcePath}`);
    const extractedEntries = await readdir(extractionRoot, { withFileTypes: true });
    const visibleEntries = extractedEntries.filter((entry) => !entry.name.startsWith('.'));
    const resolvedRoot = visibleEntries.length === 1 && visibleEntries[0].isDirectory()
      ? join(extractionRoot, visibleEntries[0].name)
      : extractionRoot;
    return {
      inputPath: resolvedSourcePath,
      kind: 'archive',
...
async function resolveMigrationAuditTargetRoot(options = {}) {
  const explicitRoot = trimString(options.instanceRoot);
  if (explicitRoot) {
  ...
  const normalizedName = sanitizeGuestInstanceName(options.name);
  if (!normalizedName) {
    throw new Error('Migration audit requires <name> or --instance-root');
  }

  const registry = await loadGuestRegistry();
  const record = registry.find((entry) => entry.name === normalizedName);
  if (!record?.instanceRoot) {
    throw new Error(`Guest instance not found in registry: ${normalizedName}`);
  }
  return resolve(record.instanceRoot);
}

function shouldIncludeMigrationAuditPath(relativePath) {
  const normalizedPath = trimString(relativePath);
  if (!normalizedPath) return false;
  return normalizedPath.startsWith('memory/')
    || normalizedPath.startsWith('workspace/')
    || normalizedPath.startsWith('shared-tools/')
    || normalizedPath.startsWith('config/chat-history/')
    || normalizedPath.startsWith('config/file-assets/')
    || normalizedPath.startsWith('config/images/')
    || normalizedPath.startsWith('config/tool-overlays/')
    || normalizedPath === 'config/tools.json'
    || normalizedPath === 'config/tools-enabled.md'
    || normalizedPath === 'config/ui-runtime-selection.json';
}

function shouldScanMigrationAuditTextPath(relativePath) {
  const normalizedPath = trimString(relativePath);
  if (!normalizedPath || !shouldIncludeMigrationAuditPath(normalizedPath)) return false;
  return normalizedPath.startsWith('memory/')
    || normalizedPath.startsWith('workspace/')
    || normalizedPath.startsWith('shared-tools/')
    || normalizedPath.startsWith('config/tool-overlays/')
...
function isMigrationAuditTextPath(relativePath) {
  const normalizedPath = trimString(relativePath);
  if (!normalizedPath) return false;
  const extension = trimString(normalizedPath.slice(normalizedPath.lastIndexOf('.'))).toLowerCase();
  if (MIGRATION_AUDIT_TEXT_EXTENSIONS.has(extension)) return true;
  const baseName = trimString(basename(normalizedPath)).toLowerCase();
  return ['dockerfile', 'makefile'].includes(baseName);
}

function isLikelyCodePath(relativePath) {
  const normalizedPath = trimString(relativePath);
  if (!normalizedPath) return false;
  const extension = trimString(normalizedPath.slice(normalizedPath.lastIndexOf('.'))).toLowerCase();
  if (MIGRATION_AUDIT_CODE_EXTENSIONS.has(extension)) return true;
  const baseName = trimString(basename(normalizedPath)).toLowerCase();
  return ['dockerfile', 'makefile'].includes(baseName);
}

function formatAuditBytes(value) {
...
function isPathWithinRoot(candidatePath, rootPath) {
  const normalizedCandidate = trimString(candidatePath);
  const normalizedRoot = trimString(rootPath);
  if (!normalizedCandidate || !normalizedRoot) return false;
  return normalizedCandidate === normalizedRoot || normalizedCandidate.startsWith(`${normalizedRoot}${sep}`);
}

function isMigrationAuditEphemeralPath(pathValue) {
  const normalizedPathValue = trimString(pathValue);
  if (!normalizedPathValue) return false;
  return MIGRATION_AUDIT_EPHEMERAL_PATH_PATTERNS.some((pattern) => pattern.test(normalizedPathValue));
}

function isExpectedMigrationAuditDriftPath(relativePath) {
  const normalizedPath = trimString(relativePath);
  if (!normalizedPath) return false;
  return MIGRATION_AUDIT_EXPECTED_DRIFT_PATH_PATTERNS.some((pattern) => pattern.test(normalizedPath));
}

function mapLegacyAuditPathToProjectRoot(pathValue) {
  const normalizedPathValue = trimString(pathValue);
  if (!normalizedPathValue) return null;
  const match = normalizedPathValue.match(MIGRATION_AUDIT_LEGACY_REMOTELAB_REPO_PATTERN);
  if (!match) return null;
  const projectRelativePath = trimString(match[1] || '');
  return {
    kind: 'known_project_root',
    sourceRelativePath: projectRelativePath,
    mappedTargetPath: projectRelativePath ? join(PROJECT_ROOT, projectRelativePath) : PROJECT_ROOT,
  };
}

function inferSourceInstanceRootCandidates(name, sourceRoot, sessionFolders = []) {
  const normalizedName = sanitizeGuestInstanceName(name);
  const candidates = [];
  if (trimString(sourceRoot)) {
    candidates.push(resolve(sourceRoot));
  }

  if (normalizedName) {
    const marker = `${sep}.remotelab${sep}instances${sep}${normalizedName}`;
    for (const folder of sessionFolders) {
      const normalizedFolder = trimString(folder);
      if (!normalizedFolder) continue;
      const markerIndex = normalizedFolder.indexOf(marker);
      if (markerIndex === -1) continue;
      candidates.push(normalizedFolder.slice(0, markerIndex + marker.length));
    }
  }
...
function mapAuditPathToTarget(pathValue, sourceRoots = [], targetRoot = '') {
  const normalizedPathValue = trimString(pathValue);
  ...
  for (const sourceRoot of sourceRoots) {
    if (!isPathWithinRoot(normalizedPathValue, sourceRoot)) continue;
    const sourceRelativePath = relative(sourceRoot, normalizedPathValue);
    return {
      kind: 'inside_instance_root',
      sourceRelativePath,
      mappedTargetPath: sourceRelativePath ? join(targetRoot, sourceRelativePath) : targetRoot,
    };
  }

  const projectRootMapping = mapLegacyAuditPathToProjectRoot(normalizedPathValue);
  if (projectRootMapping) {
    return projectRootMapping;
  }

  if (isMigrationAuditEphemeralPath(normalizedPathValue)) {
    return {
      kind: 'ephemeral_absolute',
      sourceRelativePath: '',
      mappedTargetPath: '',
    };
  }
  ...
  return {
    kind: normalizedPathValue.startsWith('/') ? 'external_absolute' : 'non_absolute',
    sourceRelativePath: '',
    mappedTargetPath: '',
  };
}
...
async function collectMigrationAuditEntries(rootPath, relativePath = '') {
  const currentPath = relativePath ? join(rootPath, relativePath) : rootPath;
  const dirEntries = await readdir(currentPath, { withFileTypes: true });
  const sortedEntries = dirEntries.slice().sort((left, right) => naturalCompare(left.name, right.name));
  const files = [];

  for (const entry of sortedEntries) {
    const nextRelativePath = relativePath ? join(relativePath, entry.name) : entry.name;
    const absolutePath = join(rootPath, nextRelativePath);
    if (entry.isDirectory()) {
      files.push(...await collectMigrationAuditEntries(rootPath, nextRelativePath));
      continue;
    }
    if (entry.isFile()) {
...
function sumAuditFileBytes(entries = []) {
  return entries.reduce((total, entry) => total + (entry?.kind === 'file' ? Number(entry.size || 0) : 0), 0);
}

function buildAuditEntryMap(entries = []) {
  return new Map(entries.map((entry) => [entry.path, entry]));
}

async function readInstanceSessionRecords(instanceRoot) {
  return normalizeGuestSessionRecords(
    await readJsonFile(join(instanceRoot, 'config', 'chat-sessions.json'), []),
  );
}

async function buildSessionFolderAudit(sessionRecords = [], { name = '', sourceRoot = '', targetRoot = '' } = {}) {
  const sessionFolders = [...new Set(
    sessionRecords
      .map((record) => trimString(record?.folder))
      .filter(Boolean),
  )].sort(naturalCompare);
  const sourceRoots = inferSourceInstanceRootCandidates(name, sourceRoot, sessionFolders);

  return Promise.all(sessionFolders.map(async (folder) => {
    const mapping = mapAuditPathToTarget(folder, sourceRoots, targetRoot);
    if (mapping.kind === 'inside_instance_root' || mapping.kind === 'known_project_root') {
      const existsOnTarget = await pathExists(mapping.mappedTargetPath);
      return {
        folder,
        kind: mapping.kind,
        sourceRelativePath: mapping.sourceRelativePath,
        mappedTargetPath: mapping.mappedTargetPath,
        existsOnTarget,
        status: existsOnTarget ? 'mapped_present' : 'mapped_missing',
      };
    }
    const existsOnTarget = ['external_absolute', 'ephemeral_absolute'].includes(mapping.kind)
    ...
    return {
      ...
      mappedTargetPath: '',
      existsOnTarget,
      status: mapping.kind === 'ephemeral_absolute'
        ? (existsOnTarget ? 'ephemeral_present' : 'ephemeral_missing')
        : mapping.kind === 'external_absolute'
          ? (existsOnTarget ? 'external_present' : 'external_missing')
          : 'non_absolute_reference',
    };
  }));
}

async function collectMigrationAuditPathReferences(sourceRoot, sourceEntries = [], {
  name = '',
  targetRoot = '',
  sourceRoots = [],
} = {}) {
  const sourceRootCandidates = Array.isArray(sourceRoots) && sourceRoots.length > 0
    ? [...new Set(sourceRoots.map((value) => trimString(value)).filter(Boolean))]
    : inferSourceInstanceRootCandidates(name, sourceRoot, []);
  const references = [];
  const seen = new Set();

  for (const entry of sourceEntries) {
    if (entry?.kind !== 'file') continue;
    if (!shouldScanMigrationAuditTextPath(entry.path) || !isMigrationAuditTextPath(entry.path)) continue;
    const absolutePath = join(sourceRoot, entry.path);
    const rawContent = await readFile(absolutePath, 'utf8').catch(() => '');
    if (!rawContent || rawContent.includes('\u0000')) continue;
    const matches = rawContent.match(MIGRATION_AUDIT_PATH_REFERENCE_PATTERN) || [];
    for (const match of matches) {
      const normalizedMatch = stripTrailingPathPunctuation(match);
      if (!normalizedMatch) continue;
      const dedupeKey = `${entry.path}::${normalizedMatch}`;
      if (seen.has(dedupeKey)) continue;
      seen.add(dedupeKey);
      const mapping = mapAuditPathToTarget(normalizedMatch, sourceRootCandidates, targetRoot);
      const existsOnTarget = ['inside_instance_root', 'known_project_root'].includes(mapping.kind)
        ? await pathExists(mapping.mappedTargetPath)
        : ['external_absolute', 'ephemeral_absolute'].includes(mapping.kind)
          ? await pathExists(normalizedMatch)
          : false;
      references.push({
        file: entry.path,
        ...
        mappedTargetPath: mapping.mappedTargetPath,
        existsOnTarget,
        status: ['inside_instance_root', 'known_project_root'].includes(mapping.kind)
          ? (existsOnTarget ? 'mapped_present' : 'mapped_missing')
          : mapping.kind === 'ephemeral_absolute'
            ? (existsOnTarget ? 'ephemeral_present' : 'ephemeral_missing')
            : mapping.kind === 'external_absolute'
              ? (existsOnTarget ? 'external_present' : 'external_missing')
              : 'non_absolute_reference',
  ...
  return references.sort((left, right) => {
    const fileOrder = naturalCompare(left.file, right.file);
    if (fileOrder !== 0) return fileOrder;
    return naturalCompare(left.path, right.path);
  });
}

function formatAuditPathSample(items = [], pick = (item) => item, limit = MIGRATION_AUDIT_MAX_SAMPLE_ITEMS) {
  if (!Array.isArray(items) || items.length === 0) return [];
  const sample = items.slice(0, limit).map((item) => pick(item)).filter(Boolean);
  if (items.length > limit) {
    sample.push(`... and ${items.length - limit} more`);
  }
  return sample;
}

function formatGuestInstanceMigrationAuditResult(result = {}) {
  const summary = result?.summary || {};
  const lines = [
    `name: ${trimString(result?.name) || '(unscoped)'}`,
    `status: ${summary.safeToRetireSource ? 'ready-to-retire-source' : 'needs-review'}`,
    `source: ${trimString(result?.source?.inputPath) || trimString(result?.source?.root) || '(unknown)'}`,
    `target: ${trimString(result?.target?.root) || '(unknown)'}`,
    `portableFiles: ${summary.sourcePortableFileCount || 0} source / ${summary.targetPortableFileCount || 0} target`,
    `portableBytes: ${formatAuditBytes(summary.sourcePortableBytes)} source / ${formatAuditBytes(summary.targetPortableBytes)} target`,
    `missingPortableFiles: ${summary.missingPortableFileCount || 0}`,
    `contentMismatches: ${summary.contentMismatchCount || 0}`,
    `blockingContentMismatches: ${summary.blockingContentMismatchCount || 0}`,
    `missingSessions: ${summary.missingSessionCount || 0}`,
    `missingMappedSessionFolders: ${summary.missingMappedSessionFolderCount || 0}`,
    `unresolvedExternalSessionFolders: ${summary.externalMissingSessionFolderCount || 0}`,
    `unresolvedPathReferences: ${summary.externalMissingPathReferenceCount || 0}`,
    `missingLikelyCodeFiles: ${summary.missingLikelyCodeFileCount || 0}`,
  ];

  for (const item of formatAuditPathSample(result?.missingPortableFiles, (entry) => `missingFile: ${entry.path}`)) {
    lines.push(item);
  }
  for (const item of formatAuditPathSample(result?.contentMismatches, (entry) => `mismatch: ${entry.path}`)) {
    lines.push(item);
  }
  for (const item of formatAuditPathSample(result?.missingSessions, (entry) => `missingSession: ${entry.id}${entry.name ? ` (${entry.name})` : ''}`)) {
    lines.push(item);
  }
  for (const item of formatAuditPathSample(
    (result?.sessionFolders || []).filter((entry) => entry.status === 'external_missing'),
    (entry) => `externalSessionFolder: ${entry.folder}`,
  )) {
    lines.push(item);
  }
  for (const item of formatAuditPathSample(
    (result?.pathReferences || []).filter((entry) => entry.status === 'external_missing'),
    (entry) => `externalPathRef: ${entry.path} (${entry.file})`,
  )) {
    lines.push(item);
  }
...
export async function auditGuestInstanceMigration(options = {}) {
  const source = await resolveMigrationAuditSource(options.source);
  try {
    const targetRoot = await resolveMigrationAuditTargetRoot(options);
    if (!await pathExists(targetRoot)) {
      throw new Error(`Migration audit target root does not exist: ${targetRoot}`);
    }
    const auditName = sanitizeGuestInstanceName(options.name) || sanitizeGuestInstanceName(basename(targetRoot));

    const [sourceEntriesRaw, targetEntriesRaw, sourceSessions, targetSessions] = await Promise.all([
      collectMigrationAuditEntries(source.root),
      collectMigrationAuditEntries(targetRoot),
      readInstanceSessionRecords(source.root),
      readInstanceSessionRecords(targetRoot),
    ]);
    const sourceEntries = sourceEntriesRaw.filter((entry) => shouldIncludeMigrationAuditPath(entry.path));
    const targetEntries = targetEntriesRaw.filter((entry) => shouldIncludeMigrationAuditPath(entry.path));
    const sourceEntryMap = buildAuditEntryMap(sourceEntries);
    const targetEntryMap = buildAuditEntryMap(targetEntries);

    const missingPortableFiles = [];
    const contentMismatches = [];
    for (const [relativePath, sourceEntry] of sourceEntryMap.entries()) {
      const targetEntry = targetEntryMap.get(relativePath);
      if (!targetEntry) {
        missingPortableFiles.push(sourceEntry);
        continue;
      }
      ...
      if (sourceEntry.kind === 'file' && (sourceEntry.size !== targetEntry.size || sourceEntry.sha256 !== targetEntry.sha256)) {
        contentMismatches.push({
          path: relativePath,
          sourceKind: sourceEntry.kind,
          targetKind: targetEntry.kind,
      ...
      if (sourceEntry.kind === 'symlink' && sourceEntry.target !== targetEntry.target) {
        contentMismatches.push({
          ...
          targetTarget: targetEntry.target,
        });
      }
    }

    const extraTargetFiles = [...targetEntryMap.values()]
      .filter((entry) => !sourceEntryMap.has(entry.path))
      .sort((left, right) => naturalCompare(left.path, right.path));

    const sourceSessionIds = new Map(
      sourceSessions
        .map((record) => [trimString(record?.id), record])
        .filter(([id]) => id),
    );
    const targetSessionIds = new Set(
      targetSessions
        .map((record) => trimString(record?.id))
        .filter(Boolean),
    );
    const missingSessions = [...sourceSessionIds.entries()]
      .filter(([id]) => !targetSessionIds.has(id))
      .map(([id, record]) => ({
        id,
        name: trimString(record?.name),
        folder: trimString(record?.folder),
      }))
      .sort((left, right) => naturalCompare(left.id, right.id));

    const sourceSessionFolderValues = [...new Set(
      sourceSessions
        .map((record) => trimString(record?.folder))
        .filter(Boolean),
    )];
    const sourceRootCandidates = inferSourceInstanceRootCandidates(auditName, source.root, sourceSessionFolderValues);
    const sessionFolders = await buildSessionFolderAudit(sourceSessions, {
      name: auditName,
      sourceRoot: source.root,
      targetRoot,
    });
    const pathReferences = await collectMigrationAuditPathReferences(source.root, sourceEntries, {
      name: auditName,
      targetRoot,
      sourceRoots: sourceRootCandidates,
    });
    const missingLikelyCodeFiles = missingPortableFiles
      .filter((entry) => isLikelyCodePath(entry.path))
      .sort((left, right) => naturalCompare(left.path, right.path));
    const blockingContentMismatches = contentMismatches.filter((entry) => !isExpectedMigrationAuditDriftPath(entry.path));

    const summary = {
      safeToRetireSource: missingPortableFiles.length === 0
        && blockingContentMismatches.length === 0
        && missingSessions.length === 0
        && sessionFolders.every((entry) => !['mapped_missing', 'external_missing'].includes(entry.status))
        && pathReferences.every((entry) => !['mapped_missing', 'external_missing'].includes(entry.status)),
      sourcePortableFileCount: sourceEntries.length,
      targetPortableFileCount: targetEntries.length,
      sourcePortableBytes: sumAuditFileBytes(sourceEntries),
      targetPortableBytes: sumAuditFileBytes(targetEntries),
      missingPortableFileCount: missingPortableFiles.length,
      contentMismatchCount: contentMismatches.length,
      blockingContentMismatchCount: blockingContentMismatches.length,
      extraTargetFileCount: extraTargetFiles.length,
      ...
      missingMappedSessionFolderCount: sessionFolders.filter((entry) => entry.status === 'mapped_missing').length,
      externalMissingSessionFolderCount: sessionFolders.filter((entry) => entry.status === 'external_missing').length,
      mappedMissingPathReferenceCount: pathReferences.filter((entry) => entry.status === 'mapped_missing').length,
      externalMissingPathReferenceCount: pathReferences.filter((entry) => entry.status === 'external_missing').length,
      missingLikelyCodeFileCount: missingLikelyCodeFiles.length,
    };

    return {
      name: auditName || trimString(basename(targetRoot)),
      source: {
...
function resolveGuestReportNowMs(options = {}) {
  if (Number.isFinite(options.nowMs)) return options.nowMs;
  const envNow = Date.parse(trimString(process.env.REMOTELAB_GUEST_REPORT_NOW));
  if (Number.isFinite(envNow)) return envNow;
  return Date.now();
}
...
async function queryFallbackUsageFromRunSpools({
  configDir = '',
  ...
  const startMs = normalizedEndMs - (Math.max(1, Number(days) || DEFAULT_GUEST_USAGE_WINDOW_DAYS) * 24 * 60 * 60 * 1000);
  ...
  for (const runDir of runDirs) {
    ...
    if (!record) continue;
    if (record.timestampMs < startMs || record.timestampMs > normalizedEndMs) continue;
...
async function readGuestUsageSnapshot(record = {}, { nowMs = Date.now() } = {}) {
...
function renderGuestInstanceReportMarkdown(report = {}) {
  const lines = [
    ...
    '',
    '## Instance Table',
    '| Name | Status | Brief | Chats | Access | Last Chat | 30d Tokens | 30d Cost | Top Tools | Open | Bridge |',
    '| --- | --- | --- | ---: | ---: | --- | ---: | ---: | --- | --- | --- |',
  ];
...
async function generateGuestInstanceReport(options = {}) {
  const normalizedName = sanitizeGuestInstanceName(options?.name);
  const nowMs = resolveGuestReportNowMs(options);
  ...
  const instances = await Promise.all(discoveredLinks.map(async (record) => {
    const usageSnapshot = await readGuestUsageSnapshot(record, { nowMs });
    const snapshot = {
      ...record,
      ...usageSnapshot,
    };
...
async function createSingleGuestInstance(options) {
  if (process.platform !== 'darwin') {
    const registry = await loadGuestRegistry();
    const name = resolveRequestedGuestName(options, { registry, isolatedAgents: [] });
    if (!name) {
    ...
    const cloudflaredContent = options.localOnly ? '' : await readCloudflaredContent();
    const port = await chooseGuestPort(options, registry, cloudflaredContent, []);
    const isolated = options.isolated === true || GUEST_SYSTEMD_ISOLATION_ENABLED;

    // Provision dedicated OS user + systemd sandbox before creating instance layout
    let isolatedUser = '';
    let instanceHome = '';
    if (isolated) {
      const provisioned = await provisionIsolatedInstance(name);
      isolatedUser = provisioned.userName;
      instanceHome = provisioned.homePath;
    }

    const instanceRoot = isolated
      ? instanceHome
      : resolve(trimString(options.instanceRoot) || join(DEFAULT_GUEST_INSTANCES_ROOT, name));
    ...
    const hostname = options.localOnly
      ...
      : trimString(options.hostname)
        || deriveGuestHostname(cloudflaredContent, {
          ...
          domain: options.domain || guestDefaults.publicDomain,
          ownerPort: OWNER_PORT,
        });
    if (!options.localOnly && !hostname) {
      throw new Error('Could not derive a public hostname; pass --hostname or configure guest-instance publicDomain');
    }

    await ensureGuestLayout({ name, hostname, instanceRoot, configDir, memoryDir });
    await syncGuestPlatformSkills(memoryDir);
    await seedGuestRuntimeTools(configDir);
    ...
    if (!isolated) {
      await seedGuestToolCredentials(instanceRoot);
    }

    // For isolated instances, set file ownership after writing layout + auth
    if (isolated) {
      await execOrThrow('chown', ['-R', `${isolatedUser}:${isolatedUser}`, instanceRoot],
        `Setting instance ownership for ${name}`);
    }
    ...
    const guestPathValue = buildGuestPathValue({
      homeDir: instanceRoot,
      basePath: process.env.PATH || '',
    });
    ...
    const envFile = await writeLinuxGuestEnvironmentFile(name, {
      NODE_ENV: 'production',
      CHAT_BIND_HOST: DEFAULT_GUEST_CHAT_BIND_HOST,
      CHAT_PORT: String(port),
      ...runtimeCellEnvironment,
      TMPDIR: join(instanceRoot, 'tmp'),
      PATH: guestPathValue,
      ...(publicBaseUrl ? { REMOTELAB_PUBLIC_BASE_URL: publicBaseUrl } : {}),
      REMOTELAB_USER_SHELL_ENV_B64: buildGuestShellEnvSnapshot({
        homeDir: instanceRoot,
        userName: isolated ? isolatedUser : '',
        basePath: guestPathValue,
        workingDirectory: PROJECT_ROOT,
      }),
    ...
    // Ensure env file directory is not listable by guest users
    if (isolated) {
      await execOrThrow('chmod', ['700', LINUX_GUEST_ENV_DIR],
        'Restricting guest env directory listing');
    }

    const serviceName = await startLinuxGuestService(name);
    ...
    const record = normalizeGuestInstanceRecord({
      ...
      authFile,
      launchAgentPath: envFile,
      logPath: join(LINUX_LOG_DIR, `chat-server-${name}.log`),
      errorLogPath: join(LINUX_LOG_DIR, `chat-server-${name}.error.log`),
      localBaseUrl,
      publicBaseUrl,
    ...
    try {
      await runBootstrapBackfill({ instanceRoot, port });
    } catch (error) {
      warnings.push(`Starter session bootstrap failed for ${name}: ${error instanceof Error ? error.message : String(error)}`);
    }

    try {
      await seedGuestWeChatConnectorConfig(configDir, { localBaseUrl });
    } catch (error) {
      warnings.push(`WeChat connector setup failed for ${name}: ${error instanceof Error ? error.message : String(error)}`);
    }
...
async function createGuestInstance(options) {
  const count = Number.isInteger(options?.count) && options.count > 0 ? options.count : 1;
  if (count === 1) {
    return createSingleGuestInstance(options);
  }

  const registry = await loadGuestRegistry();
  const isolatedAgents = process.platform === 'darwin'
    ? await discoverIsolatedLaunchAgents()
    : [];
  const plannedNames = planGuestInstanceCreateNames(options, { registry, isolatedAgents });
  ...
  for (const name of plannedNames) {
    try {
      const created = await createSingleGuestInstance({
        ...options,
        name,
        count: 1,
        trial: false,
      ...
      const createdNames = results.map((record) => record.name).filter(Boolean);
      const prefix = createdNames.length > 0
        ? `Batch create failed at ${name} after creating ${createdNames.join(', ')}: `
        : `Batch create failed at ${name}: `;
...
async function exposeGuestInstancePort(options, { execFileFn = execFileAsync } = {}) {
  const normalizedName = sanitizeGuestInstanceName(options?.name);
  const normalizedLabel = sanitizeGuestInstanceName(options?.label);
  const port = Number.parseInt(options?.port, 10) || 0;
  if (!normalizedName) {
    throw new Error('expose requires a guest instance name');
  }
  if (!normalizedLabel) {
    throw new Error('expose requires --label <label>');
  }
  ...
  if (!trimString(record.isolatedUser)) {
    throw new Error(`Guest instance ${normalizedName} is not isolated; controlled port exposure currently requires isolatedUser metadata`);
  }

  const nextRoute = buildGuestExposedRouteRecord(record, {
    label: normalizedLabel,
    port,
  });
  const routeKey = buildGuestRouteKey(nextRoute);
  const lookup = collectGuestRouteLookupConflicts({
    registry,
    routes,
    ignoreRouteKey: routeKey,
  });
  const hostLabel = firstHostnameLabel(nextRoute.hostname);
  const conflictingHostname = lookup.usedHostnames.get(nextRoute.hostname);
  if (conflictingHostname) {
    throw new Error(`Hostname ${nextRoute.hostname} is already reserved by ${conflictingHostname}`);
  }
  if (lookup.reservedLabels.has(hostLabel)) {
    throw new Error(`Hostname label ${hostLabel} is reserved by the host router`);
  }
  const conflictingLabel = lookup.usedLabels.get(hostLabel);
  if (conflictingLabel) {
    throw new Error(`Hostname label ${hostLabel} would collide with ${conflictingLabel}`);
  }

  const inspection = await inspectListeningPort(port, { execFileFn });
  if (!inspection.listening) {
    throw new Error(`Port ${port} is not listening`);
  }
  if (!inspection.loopbackOnly) {
    throw new Error(`Port ${port} must listen on loopback only before it can be exposed`);
  }
  const currentInstanceRoot = trimString(process.env.REMOTELAB_INSTANCE_ROOT);
  const isRootInSameInstance = process.getuid?.() === 0
    && currentInstanceRoot
    && resolve(currentInstanceRoot) === resolve(record.instanceRoot);
  const allowedOwners = new Set([
    record.isolatedUser,
    ...(isRootInSameInstance ? ['root'] : []),
  ]);
  const hasOnlyAllowedOwners = inspection.loginNames.length === 1
    && allowedOwners.has(inspection.loginNames[0]);
  if (!hasOnlyAllowedOwners) {
    const owners = inspection.loginNames.length > 0
      ? inspection.loginNames.join(', ')
      : 'unknown';
    throw new Error(`Port ${port} is owned by ${owners}; expected ${[...allowedOwners].join(' or ')}`);
  }

  const now = new Date().toISOString();
  const existingRoute = routes.find((route) => buildGuestRouteKey(route) === routeKey) || null;
  const persistedRoute = normalizeGuestRouteRecord({
    ...nextRoute,
    createdAt: existingRoute?.createdAt || now,
    updatedAt: now,
  });
  const nextRoutes = [
    ...routes.filter((route) => buildGuestRouteKey(route) !== routeKey),
    persistedRoute,
  ];
  await saveGuestRouteRegistry(nextRoutes);

  const token = await readTokenFromAuthFile(record.authFile);
  const publicBaseUrl = `https://${persistedRoute.hostname}`;
  return {
    command: 'expose',
    changed: !existingRoute
      || existingRoute.port !== persistedRoute.port
      || existingRoute.hostname !== persistedRoute.hostname
      || existingRoute.targetHost !== persistedRoute.targetHost,
    instanceName: normalizedName,
    label: persistedRoute.label,
    port: persistedRoute.port,
    hostname: persistedRoute.hostname,
    publicBaseUrl,
    accessUrl: token ? buildAccessUrl(publicBaseUrl, token) : publicBaseUrl,
    serviceTarget: buildGuestRouteServiceTarget(persistedRoute),
    listenerOwners: inspection.loginNames,
    listenerBindings: inspection.listeners.flatMap((entry) => entry.addresses),
    route: persistedRoute,
  };
}

async function unexposeGuestInstancePort(options) {
  const normalizedName = sanitizeGuestInstanceName(options?.name);
  const normalizedLabel = sanitizeGuestInstanceName(options?.label);
  if (!normalizedName) {
    throw new Error('unexpose requires a guest instance name');
  }
  if (!normalizedLabel) {
    throw new Error('unexpose requires --label <label>');
  }
  ...
  const routes = await loadGuestRouteRegistry();
  const routeKey = buildGuestRouteKey({
    instanceName: normalizedName,
    label: normalizedLabel,
  });
  const existingRoute = routes.find((route) => buildGuestRouteKey(route) === routeKey) || null;
  ...
  const nextRoutes = routes.filter((route) => buildGuestRouteKey(route) !== routeKey);
  await saveGuestRouteRegistry(nextRoutes);
  return {
    command: 'unexpose',
    changed: true,
    ...
    publicBaseUrl: `https://${existingRoute.hostname}`,
    serviceTarget: buildGuestRouteServiceTarget(existingRoute),
    route: existingRoute,
  };
}
...
function isIsolatedInstance(target, registry = []) {
  if (!GUEST_SYSTEMD_ISOLATION_ENABLED) return false;
  // Check registry for explicit isolatedUser marker
  const registryRecord = registry.find((record) => record.name === target.name);
  if (trimString(registryRecord?.isolatedUser)) return true;
  // Infer from HOME pointing to the isolated homes root
  const home = trimString(target.environmentVariables?.HOME);
  if (home && home.startsWith(LINUX_GUEST_HOMES_ROOT + sep)) return true;
  return false;
}
...
async function convergeGuestInstances(options) {
  ...
  if (!options.all && matchingTargets.length === 0) {
    throw new Error(`Guest instance not found: ${sanitizeGuestInstanceName(options.name)}`);
  }
  ...
  for (const target of matchingTargets) {
    ...
    const runtimeDefaults = configDir
      ? await syncGuestRuntimeTools(configDir, { dryRun: options.dryRun })
      : {
        changed: false,
        toolsChanged: false,
        selectionChanged: false,
    ...
    const mailboxPrivacy = configDir
      ? (options.dryRun
        ? { changed: false, outboundChanged: false, automationChanged: false }
        : await syncGuestMailboxPrivacyBaseline({
          name: target.name,
          configDir,
          mailboxRoot: join(configDir, 'agent-mailbox'),
    ...
    const currentLocalProbe = await fetchBuildInfo(target.localBaseUrl, 1500);
    const build = buildVersionStateFromProbes({
      ownerProbe,
      localProbe: currentLocalProbe,
      publicProbe: null,
    });
    const buildDriftChanged = build.localMatchesOwner === false;
    const result = {
      name: target.name,
      label: target.label,
      port: target.port,
      ...
      changed: drift.changed || buildDriftChanged,
      drift,
      build,
      buildDriftChanged,
      ...
      platformUpdatedSkillIds: platformSkills.updatedSkillIds,
      mailboxPrivacyChanged: mailboxPrivacy.changed,
      mailboxOutboundChanged: mailboxPrivacy.outboundChanged,
      mailboxAutomationChanged: mailboxPrivacy.automationChanged,
    ...
    const isolated = isIsolatedInstance(target, registry);
    const isolationOverridePath = join(buildSystemdOverrideDir(target.name), 'isolation.conf');
    const shouldRemoveIsolationOverride = !GUEST_SYSTEMD_ISOLATION_ENABLED
      && await pathExists(isolationOverridePath);
    if (shouldRemoveIsolationOverride) {
      result.changed = true;
    }
    if (mailboxPrivacy.changed) {
      result.changed = true;
    }
    ...
    if (drift.changed && !options.dryRun) {
      ...
      if (usesLaunchAgentRuntime(target)) {
        ...
        await writeTextAtomic(target.launchAgentPath, serializeEnvFile(next.environmentVariables));
        await execOrThrow('chmod', ['600', target.launchAgentPath],
          `Restricting env file permissions for ${target.name}`);
        await execOrThrow('chown', ['root:root', target.launchAgentPath],
          `Setting env file ownership for ${target.name}`);
      }
    }

    // Re-sync isolation override and guest-local tool credentials
    if (isolated && !options.dryRun) {
      await writeIsolationOverride(target.name);
      result.isolationSynced = true;
    }
    if (!options.dryRun && trimString(next.environmentVariables?.HOME)) {
      await seedGuestToolCredentials(trimString(next.environmentVariables.HOME), {
        userName: isolated ? buildIsolatedUserName(target.name) : '',
      });
    }

    if (shouldRemoveIsolationOverride && !options.dryRun) {
      result.isolationRemoved = await removeIsolationOverride(target.name);
    }

    if ((drift.changed || buildDriftChanged || shouldRemoveIsolationOverride) && !options.dryRun && !options.noRestart) {
      await restartGuestService(target, { reloadDefinition: drift.changed || shouldRemoveIsolationOverride });
      ...
      result.build = buildVersionStateFromProbes({
        ownerProbe,
        localProbe: localHealth,
        publicProbe: null,
      });
    ...
    if (result.localReachable === null) {
      result.localReachable = currentLocalProbe?.ok === true;
    }
...
export async function runGuestInstanceCommand(argv = [], io = {}) {
  ...
  if (options.command === 'create') {
    ...
    if (Array.isArray(result)) {
      const rendered = result.map((record) => formatGuestInstance(record, {
        token: record.token,
        localReachable: record.localReachable,
        publicReachable: record.publicReachable,
        warnings: record.warnings,
      }));
      stdout.write(`created: ${result.length}\n\n${rendered.join('\n\n')}\n`);
      return 0;
    }
  ...
  if (options.command === 'expose') {
    const result = await exposeGuestInstancePort(options);
    if (options.json) {
      stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    stdout.write(`${formatGuestExposeResult(result)}\n`);
    return 0;
  }

  if (options.command === 'unexpose') {
    const result = await unexposeGuestInstancePort(options);
    if (options.json) {
      stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    stdout.write(`${formatGuestUnexposeResult(result)}\n`);
    return 0;
  }
  ...
  if (options.command === 'check') {
    const result = await checkGuestInstanceFleet(options);
    if (options.json) {
      stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    stdout.write(`${formatGuestInstanceCheckResult(result)}\n`);
    return 0;
  }
  ...
  if (options.command === 'audit-migration') {
    const result = await auditGuestInstanceMigration(options);
    if (options.json) {
      stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    stdout.write(`${formatGuestInstanceMigrationAuditResult(result)}\n`);
    return 0;
  }
...
export {
  ...
  buildIsolatedHomePath,
  buildIsolatedUserName,
  exposeGuestInstancePort,
  formatGuestExposeResult,
  formatGuestInstance,
  formatGuestInstanceLinks,
  formatGuestInstanceMigrationAuditResult,
  formatGuestUnexposeResult,
