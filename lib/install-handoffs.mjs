...
function normalizeStoredInstallHandoff(record) {
  ...
  return {
    expiry,
    personId: normalizePersonId(record.personId),
    preferredLanguage: normalizePreferredLanguage(record.preferredLanguage),
  };
}

async function loadInstallHandoffs() {
  try {
    let raw;
    try {
      raw = JSON.parse(await readFile(INSTALL_HANDOFFS_FILE, 'utf8'));
    } catch (readError) {
    ...
    if (pruned) {
      ...
      await writeFile(INSTALL_HANDOFFS_FILE, JSON.stringify(Object.fromEntries(map), null, 2), 'utf8');
    }
    return map;
  } catch (error) {
...
export const installHandoffs = await loadInstallHandoffs();
...
export async function createInstallHandoff(authSession, { ttlMs = INSTALL_HANDOFF_TTL_MS } = {}) {
  if (!authSession) {
    throw new Error('Install handoff requires an authenticated session');
  }
  ...
  const record = {
    expiry,
    personId: normalizePersonId(authSession.personId),
