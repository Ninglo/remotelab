...
export function sameConversation(left, right) {
  ...
  if (a.connector === 'feishu') {
    ...
    const xMain = x.conversationKind === 'main', yMain = y.conversationKind === 'main';
    if (xMain || yMain) return xMain && yMain;
    const xTopicIds = topicIds(x), yTopicIds = topicIds(y);
    const ids = new Set(xTopicIds);
    if (yTopicIds.some(id => ids.has(id))) return true;
    // Pre-topology private-chat records remain readable until an operator runs
    // the explicit migration, but new main and thread Sessions are always
    // distinguished by conversationKind above.
    return !xTopicIds.length && !yTopicIds.length
      && ['p2p', 'private'].includes(x.chatType) && ['p2p', 'private'].includes(y.chatType);
  }
...
export function conversationAfterReceipt(value, receipt = {}) {
  ...
  if (conversation.connector !== 'feishu' || target.commentId || target.conversationKind === 'main') return conversation;
  // Legacy unthreaded reply snapshots remain inline. New Sessions make the
  // topology explicit with conversationKind=main|thread.
