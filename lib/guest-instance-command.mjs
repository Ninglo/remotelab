...
const DEFAULT_RELEASE_MANAGED_GUEST_HEALTH_TIMEOUT_MS = 30_000;
...
function buildInfoMatchesExpectedRelease(buildInfo, expectedReleaseId) {
  const runtimeMode = trimString(buildInfo?.runtimeMode);
  const releaseId = trimString(buildInfo?.releaseId);
  if (expectedReleaseId === null) {
    return !releaseId && runtimeMode === 'source';
  }
  if (typeof expectedReleaseId === 'string' && expectedReleaseId.trim()) {
    return runtimeMode === 'release' && releaseId === expectedReleaseId.trim();
  }
  return true;
}

async function waitForGuestReleaseHealth(baseUrl, { timeoutMs = DEFAULT_RELEASE_MANAGED_GUEST_HEALTH_TIMEOUT_MS, intervalMs = 750, expectedReleaseId } = {}) {
  const deadline = Date.now() + timeoutMs;
  let lastError = '';

  while (Date.now() < deadline) {
    const buildInfoResult = await fetchBuildInfo(baseUrl, Math.max(2000, intervalMs * 4));
    if (buildInfoResult?.ok) {
      const buildInfo = buildInfoResult.body;
      if (buildInfoMatchesExpectedRelease(buildInfo, expectedReleaseId)) {
        try {
      ...
      lastError = buildInfoResult?.error
        || `build-info returned ${buildInfoResult?.status || 0}`;
    }
...
function isReleaseManagedGuestInstance(agent = {}) {
  return trimString(agent?.environmentVariables?.REMOTELAB_ENABLE_ACTIVE_RELEASE) === '1';
}

async function listReleaseManagedGuestInstances() {
  const targets = await discoverIsolatedLaunchAgents();
  return targets.filter((target) => isReleaseManagedGuestInstance(target));
}

async function restartReleaseManagedGuestInstances(options = {}) {
  const targets = await listReleaseManagedGuestInstances();
  const timeoutMs = Number.isInteger(options.timeoutMs) && options.timeoutMs > 0
    ? options.timeoutMs
    : DEFAULT_RELEASE_MANAGED_GUEST_HEALTH_TIMEOUT_MS;
  const hasExpectedReleaseId = Object.prototype.hasOwnProperty.call(options, 'expectedReleaseId');
  const results = [];
  ...
  for (const target of targets) {
    ...
    if (!options.skipHealth) {
      const healthResult = await waitForGuestReleaseHealth(target.localBaseUrl, {
        timeoutMs,
        intervalMs: 750,
        ...(hasExpectedReleaseId ? { expectedReleaseId: options.expectedReleaseId } : {}),
      });
      result.localReachable = true;
      result.releaseId = trimString(healthResult?.buildInfo?.releaseId);
      result.runtimeMode = trimString(healthResult?.buildInfo?.runtimeMode);
    }
...
export {
  ...
  formatGuestInstance,
  listReleaseManagedGuestInstances,
  restartReleaseManagedGuestInstances,
};
