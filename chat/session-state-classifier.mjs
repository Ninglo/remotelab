...
async function runSessionStateSuggestion(sessionMeta, _options = {}) {
  ...
  const prompt = [
    ...
    '- Do not create a Project merely to preserve the specificity of one turn; keep that specificity in the Session title and description.',
    ...
    '- Use "waiting_user" only for a hard blocker: missing authorization/credential/file/input that cannot be inferred, or an unscoped irreversible high-impact decision. Never use it for optional review or a low-loss, reversible, auditable action; execute and report those instead. Use "done" only when the current goal is clearly delivered. Use "parked" only when work is intentionally deferred.',
    ...
    promptContext.existingSessions ? `Current active Space → Project hierarchy:\n${promptContext.existingSessions}` : '',
    '',
