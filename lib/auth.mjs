...
import { findActiveTeamSessionAccountByUsername } from '../chat/team-session-view.mjs';
...
export function getAdminAuthSessionFields(currentAuth = auth) {
  const username = trimString(currentAuth?.username);
  return {
    accountId: 'owner',
    accountName: username || 'Owner',
...
export async function authenticatePasswordAsync(username, password) {
  ...
  if (
    ...
    && await verifyPasswordHashAsync(password, currentAuth.passwordHash)
  ) {
    return getAdminAuthSessionFields(currentAuth);
  }
  const account = await findActiveTeamSessionAccountByUsername(username);
  if (!account || !await verifyPasswordHashAsync(password, account.passwordHash)) {
    return null;
  }
  return {
...
export async function verifyPasswordAsync(username, password) {
  return !!await authenticatePasswordAsync(username, password);
}
