...
const rootUrl = `https://smith.langchain.com/o/workspace/projects/p/${projectId}/trace/${traceId}/run/${traceId}`;
...
try {
  ...
  assert.equal(destination.headers.get('location'), rootUrl.replace(`/run/${traceId}`, `/run/${childId}`));
