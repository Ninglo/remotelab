...
Failed originals remain in active Request state until explicitly retried, confirmed delivered, or dismissed through the existing resolution API. Session detail projects the current issue list; session summaries carry only the count, preserving their cache contract. Pending deliveries older than two minutes show a connection/delay warning even when no sender is running. Retry progress, expired sends, failures and uncertainty appear above the composer and in the session list without changing chronological ordering.
