...
import { restartReleaseManagedGuestInstances } from './guest-instance-command.mjs';
...
async function rolloutReleaseManagedGuestInstances(options = {}) {
  process.stdout.write('Rolling release-managed guest instances...\n');
  const results = await restartReleaseManagedGuestInstances({
    timeoutMs: options.timeoutMs,
    skipHealth: options.skipHealth,
    ...(Object.prototype.hasOwnProperty.call(options, 'expectedReleaseId')
      ? { expectedReleaseId: options.expectedReleaseId }
      : {}),
    onProgress(result) {
      process.stdout.write(`  ${result.name}: restarted`);
      if (result.localBaseUrl) {
        process.stdout.write(` (${result.localBaseUrl}`);
        if (result.publicBaseUrl) {
          process.stdout.write(` -> ${result.publicBaseUrl}`);
        }
        process.stdout.write(')');
      }
      process.stdout.write('\n');
...
async function rollbackToManifest(targetManifest, options = {}) {
  ...
  if (options.skipHealth) {
    return rolloutReleaseManagedGuestInstances({
      timeoutMs: options.timeoutMs,
      skipHealth: true,
      expectedReleaseId: targetManifest ? targetManifest.releaseId : null,
    });
  }
  const ownerHealth = await waitForHealthyRelease({
    baseUrl: options.baseUrl,
    timeoutMs: options.timeoutMs,
    expectedReleaseId: targetManifest ? targetManifest.releaseId : null,
  });
  const guestResults = await rolloutReleaseManagedGuestInstances({
    timeoutMs: options.timeoutMs,
    skipHealth: false,
    expectedReleaseId: targetManifest ? targetManifest.releaseId : null,
  });
  return { ownerHealth, guestResults };
}
...
async function runCreateRelease(options) {
  ...
  try {
    ...
    let guestResults = [];
    if (!options.skipRestart) {
      guestResults = await rolloutReleaseManagedGuestInstances({
        timeoutMs: options.timeoutMs,
        skipHealth: options.skipHealth,
        expectedReleaseId: releaseMetadata.releaseId,
      });
    ...
    if (guestResults.length > 0) {
      process.stdout.write(`Rolled ${guestResults.length} release-managed guest instance(s).\n`);
    }
    return 0;
  } catch (error) {
