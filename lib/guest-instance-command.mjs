...
import { access, copyFile, mkdir, mkdtemp, readFile, readdir, readlink, rename, rm, stat, writeFile } from 'fs/promises';
...
async function runOwnerBootstrapBackfill({ instanceRoot, port }) {
  const env = {
    ...process.env,
  };
  delete env.REMOTELAB_CONFIG_DIR;
  ...
  const result = await execFileAsync(
    process.execPath,
    [join(PROJECT_ROOT, 'scripts', 'backfill-owner-bootstrap.mjs')],
    {
      cwd: PROJECT_ROOT,
      env,
      encoding: 'utf8',
...
function shouldIncludeMigrationAuditPath(relativePath) {
  ...
  return normalizedPath.startsWith('memory/')
    ...
    || normalizedPath === 'config/apps.json';
}
...
async function createSingleGuestInstance(options) {
  if (process.platform !== 'darwin') {
    ...
    try {
      await runOwnerBootstrapBackfill({ instanceRoot, port });
    } catch (error) {
