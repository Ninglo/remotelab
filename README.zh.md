...
现在这条大陆桥默认会保留原来的根路径上游，同时把 owner 主服务额外挂到 `https://<mainland-base>/owner/`（默认本机 `7690` 端口）；guest instance 继续走 `https://<mainland-base>/<instance>/` 这种路径前缀分流。想临时把根路径切到别的实例时，用 `NATAPP_ROOT_UPSTREAM_PORT`；想改 owner 这条专用入口时，用 `NATAPP_OWNER_UPSTREAM_PORT` / `NATAPP_OWNER_ROUTE_PREFIX`。
