...
if [[ "$OS_TYPE" == "macos" ]]; then
    ...
    print_info "Linux owner chat uses a system-scoped service (remotelab.service). You may be prompted for sudo."
    sudo -v
    sudo mkdir -p "$LOG_DIR"
    sudo chown "$CURRENT_USER:$CURRENT_GROUP" "$LOG_DIR"
fi
...
STOPEOF
    ...
    # ── Linux: system-scoped owner service + user-scoped tunnel ──────────────
    SYSTEMD_USER_DIR="$HOME/.config/systemd/user"
    SYSTEMD_OWNER_UNIT="/etc/systemd/system/remotelab.service"
    OWNER_ENV_DIR="/etc/remotelab"
    OWNER_ENV_FILE="$OWNER_ENV_DIR/remotelab.env"
    mkdir -p "$SYSTEMD_USER_DIR"

    NODE_BIN=$(which node)

    print_info "Creating owner chat control-plane service..."
    sudo mkdir -p "$OWNER_ENV_DIR"
    if [[ "$USE_CLOUDFLARE" == true ]]; then
        sudo tee "$OWNER_ENV_FILE" > /dev/null << EOF
# Generated by remotelab setup
NODE_ENV=production
EOF
    else
        sudo tee "$OWNER_ENV_FILE" > /dev/null << EOF
...
EOF
    ...
    print_success "Created: /etc/remotelab/remotelab.env"

    sudo tee "$SYSTEMD_OWNER_UNIT" > /dev/null << EOF
[Unit]
Description=RemoteLab Owner Chat Control Plane
...
EOF
    print_success "Created: /etc/systemd/system/remotelab.service"

    if [[ -f "$SYSTEMD_USER_DIR/remotelab-chat.service" ]]; then
        systemctl --user stop remotelab-chat.service 2>/dev/null || true
        systemctl --user disable remotelab-chat.service 2>/dev/null || true
        rm -f "$SYSTEMD_USER_DIR/remotelab-chat.service"
        print_success "Removed legacy ~/.config/systemd/user/remotelab-chat.service"
    fi

    if [[ -f "$SYSTEMD_USER_DIR/remotelab-proxy.service" ]]; then
        ...
        rm -f "$SYSTEMD_USER_DIR/remotelab-proxy.service"
        print_success "Removed legacy remotelab-proxy.service"
    fi

    if [[ "$USE_CLOUDFLARE" == true ]]; then
        print_info "Creating cloudflared tunnel service..."
        CLOUDFLARED_BIN=$(which cloudflared)
        cat > "$SYSTEMD_USER_DIR/remotelab-tunnel.service" << EOF
[Unit]
...
if [[ "$OS_TYPE" == "macos" ]]; then
    ...
    sudo systemctl stop remotelab.service 2>/dev/null || true
    systemctl --user stop remotelab-chat.service 2>/dev/null || true
    systemctl --user stop remotelab-proxy.service 2>/dev/null || true
    if [[ "$USE_CLOUDFLARE" == true ]]; then
        systemctl --user stop remotelab-tunnel.service 2>/dev/null || true
...
if [[ "$OS_TYPE" == "macos" ]]; then
    ...
    sudo systemctl enable remotelab.service > /dev/null
    sudo systemctl start remotelab.service
    if [[ "$USE_CLOUDFLARE" == true ]]; then
    ...
    if sudo systemctl is-active --quiet remotelab.service; then
