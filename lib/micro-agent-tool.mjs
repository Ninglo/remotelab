...
import { PRODUCT_DEFAULT_CODEX_MODEL, PRODUCT_DEFAULT_CODEX_EFFORT } from './legacy-micro-agent.mjs';
...
export const MICRO_AGENT_DEFAULT_MODEL = PRODUCT_DEFAULT_CODEX_MODEL;
export const MICRO_AGENT_DEFAULT_PROVIDER = 'hybrid';
export const MICRO_AGENT_DEFAULT_REASONING_LEVELS = ['low', 'medium', 'high', 'xhigh'];
export const MICRO_AGENT_GPT56_REASONING_LEVELS = ['low', 'medium', 'high', 'xhigh', 'max', 'ultra'];
export const MICRO_AGENT_GPT56_LUNA_REASONING_LEVELS = ['low', 'medium', 'high', 'xhigh', 'max'];
export const MICRO_AGENT_DEFAULT_REASONING_LEVEL = PRODUCT_DEFAULT_CODEX_EFFORT;
export const MICRO_AGENT_CLAUDE_REASONING_LEVELS = ['low', 'medium', 'high'];
export const MICRO_AGENT_PERSONAL_CODEX_CONFIG_PATH = join(resolveCodexHomeDir(), 'config.toml');
...
function resolveCodexReasoningLevels(modelId) {
  const normalized = trimString(modelId).toLowerCase();
  if (normalized === 'gpt-6-luna' || normalized === 'gpt-5.6-luna') return MICRO_AGENT_GPT56_LUNA_REASONING_LEVELS;
  if (normalized === 'gpt-6-astra' || normalized === 'gpt-6-sol' || normalized === 'gpt-5.6' || normalized === 'gpt-5.6-sol' || normalized === 'gpt-5.6-terra') {
    return MICRO_AGENT_GPT56_REASONING_LEVELS;
  }
  return MICRO_AGENT_DEFAULT_REASONING_LEVELS;
}
...
function buildHybridModelRecords(primaryModel) {
  return [
    buildReasoningModelRecord({
      id: primaryModel,
      label: primaryModel,
      levels: resolveCodexReasoningLevels(primaryModel),
...
export async function buildMicroAgentToolRecord({
  ...
  if (normalizedProvider === 'codex') {
    const codexReasoningLevels = resolveCodexReasoningLevels(resolvedModel);
    return {
