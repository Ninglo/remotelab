...
Examples:
  remotelab connector call feishu:document_get --document-token <docx-or-wiki-token-or-url> --json
  remotelab connector call feishu:wiki_node_get --node-token <wiki-token-or-url> --json
  remotelab connector call feishu:wiki_children_list --space-id <space-id> --page-size 50 --json
  remotelab connector call feishu:wiki_tree_list --space-id <space-id> --max-depth 4 --max-nodes 500 --json

Connector calls are instance-scoped. Provider credentials remain inside the
registered connector process and are never printed by this command. Calls made
...
export async function runConnectorCommand(argv = [], io = {}) {
  ...
  for (const [name, value] of Object.entries(options)) {
    ...
    parameters[optionNameToParameter(name)] = coerceOptionValue(value);
  }
