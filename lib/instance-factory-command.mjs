...
const legacyToken = typeof auth.token === 'string' ? auth.token.trim() : '';
if (auth.version !== 2 || !Array.isArray(auth.people)) {
  const accessToken = /^[0-9a-f]{64}$/i.test(legacyToken) ? legacyToken : randomBytes(32).toString('hex');
  auth = {
    version: 2,
    ...
    people: [{
      id: 'person_default',
      name: typeof auth.username === 'string' && auth.username.trim() ? auth.username.trim() : 'Administrator',
      credentials: [
        { id: 'credential_default_token', type: 'token', token: accessToken, label: 'Primary access token', createdAt: now },
        ...(typeof auth.username === 'string' && auth.username.trim() && typeof auth.passwordHash === 'string' && auth.passwordHash.trim()
          ? [{ id: 'credential_default_password', type: 'password', username: auth.username.trim(), passwordHash: auth.passwordHash.trim(), label: auth.username.trim(), createdAt: now }]
          : []),
      ],
      identities: [{ id: 'identity_web_default', kind: 'web', realm: 'remotelab', subjectId: 'person_default', displayName: 'Administrator', discoveredAt: now }],
...
let primary = auth.people.find((person) => person?.id === auth.primaryPersonId) || auth.people[0];
if (!primary) {
  throw new Error('auth.json has no person record');
}
if (!Array.isArray(primary.credentials)) primary.credentials = [];
let tokenCredential = primary.credentials.find((credential) => credential?.type === 'token' && typeof credential.token === 'string' && credential.token.trim());
if (!tokenCredential) {
  tokenCredential = { id: 'credential_' + randomBytes(12).toString('hex'), type: 'token', token: randomBytes(32).toString('hex'), label: 'Primary access token', createdAt: now };
  primary.credentials.push(tokenCredential);
  status = 'created';
}
if (typeof auth.serviceToken !== 'string' || !auth.serviceToken.trim()) {
  auth.serviceToken = randomBytes(32).toString('hex');
}
const token = tokenCredential.token.trim();
