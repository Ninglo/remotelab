...
function buildOutputLines(trigger) {
  return [
    ...
    trimString(trigger?.sourceSessionId) ? `sourceSessionId: ${trimString(trigger.sourceSessionId)}` : '',
    trimString(trigger?.sessionId) ? `sessionId: ${trimString(trigger.sessionId)}` : '',
...
function parseArgs(argv = []) {
  const options = {
    ...
    in: '',
    at: '',
    sessionId: trimString(argv[0]).toLowerCase() === 'list' ? '' : trimString(process.env.REMOTELAB_SESSION_ID),
    title: '',
    tool: '',
    model: '',
    runtimePolicy: '',
  ...
  const valueFlags = new Set(['--text', '--in', '--at', '--session', '--source-request', '--conversation', '--conversation-file', '--title', '--tool', '--model', '--effort', '--runtime-policy', '--base-url']);
  ...
  for (let index = argsStartIndex; index < argv.length; index += 1) {
    ...
    switch (arg) {
      ...
      case '--title':
        ...
        break;
      case '--conversation':
        options.conversation = argv[++index] ?? '';
        break;
      case '--conversation-file':
        options.conversationFile = argv[++index] ?? '';
        if (!options.conversationFile) throw new Error('Missing value for --conversation-file');
        break;
      case '--source-request':
        options.sourceDelivery = true;
        options.sourceRequestId = argv[index + 1] || '';
        index += 1;
        break;
      case '--no-source-delivery':
        options.sourceDelivery = false;
        break;
      case '--tool':
      ...
      case '--model':
        ...
        break;
      case '--runtime-policy':
        options.runtimePolicy = argv[index + 1] || '';
        index += 1;
        break;
      case '--effort':
  ...
  options.sourceRequestId = trimString(options.sourceRequestId);
